...
function interpolate(template: string, values: InterpolationValues): string {
    return template.replace(/\{([^{}]+)\}/gu, (placeholder: string, name: string): string => {
...
export function discoverLocaleResources(bundledResources: Readonly<Record<string, LocaleCatalog>>): LocaleResources {
...
export function createTranslator(localeResources: LocaleResources, fallbackLocale = 'en'): Translator {
    const availableLocales: readonly string[] = Object.freeze(Object.keys(localeResources).sort());
    let activeLocale =
        localeResources[fallbackLocale] === undefined ? (availableLocales[0] ?? fallbackLocale) : fallbackLocale;
    ...
    return {
        ...
        setLocale(locale: string): void {
            activeLocale = localeResources[locale] === undefined ? fallbackLocale : locale;
        },
        t(key: string, values: InterpolationValues = {}): string {
            const activeValue = nonBlankValue(localeResources[activeLocale]?.[key]);
            const fallbackValue = nonBlankValue(localeResources[fallbackLocale]?.[key]);
