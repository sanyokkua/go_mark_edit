...
const TabBar: React.FC<TabBarProps> = ({
    ...
    const activeDrag = useRef<{ documentId: string; fromIndex: number } | null>(null);
    ...
    const handleTabListKeyDown = useCallback(
        (event: React.KeyboardEvent<HTMLDivElement>): void => {
            ...
            if (current < 0) return;
            event.preventDefault();
            const next = event.key === 'ArrowLeft' ? Math.max(0, current - 1) : Math.min(tabs.length - 1, current + 1);
                    ...
                    {tabs.flatMap((tab, index) => {
                        ...
                        const item = (
                            <div
                                ...
                                data-tab-dragging={tab.id === draggingDocumentId ? 'true' : undefined}
                                data-tab-item=""
                                key={tab.id}
                                role="presentation"
                            >
                                ...
                                >
                                    <span
                                        ...
                                        aria-label={tab.dirty ? (tab.modifiedLabel ?? 'Modified') : undefined}
                                    ...
                                    <span aria-hidden="true" className={styles.tabLabel}>
                                        {labelParts === undefined || labelParts.suffix === '' ? (
                                            ...
                                            <>
                                                <span className={styles.tabLabelBasename} data-tab-label-basename>
                                                    {labelParts.basename}
                                                </span>
                                                <span className={styles.tabLabelSuffix} data-tab-label-suffix>
                                                    {labelParts.suffix}
                                                </span>
                                            </>
                                        )}
                        ...
                        return insertionSlot === index ? [insertionIndicator(index), item] : [item];
                    })}
                    {insertionSlot === tabs.length ? insertionIndicator(tabs.length) : null}
                    <button
