...
export type PopupInitialFocus = 'first' | 'last' | 'popup' | string | HTMLElement | (() => HTMLElement | null);

export interface PopupProps extends Omit<HTMLAttributes<HTMLDivElement>, 'role'> {
...
function resolveTrigger(trigger: HTMLElement | null | RefObject<HTMLElement | null>): HTMLElement | null {
...
function anchorMetrics(
    ...
    const bounds =
        'bounds' in anchor ? anchor.bounds : (resolveTrigger(anchor.trigger)?.getBoundingClientRect() ?? new DOMRect());
    return {
...
function menuItems(popup: HTMLElement | null): HTMLElement[] {
    ...
    return Array.from(
        popup.querySelectorAll<HTMLElement>('[role="menuitem"], [role="menuitemcheckbox"], [role="menuitemradio"]'),
    ).filter(
        (item) =>
            !item.hasAttribute('disabled') && item.getAttribute('aria-disabled') !== 'true' && isVisibleMenuItem(item),
    );
}

function focusTarget(popup: HTMLElement, initialFocus: PopupInitialFocus | undefined): HTMLElement {
    if (typeof HTMLElement !== 'undefined' && initialFocus instanceof HTMLElement) return initialFocus;
    if (typeof initialFocus === 'function') {
        const target = initialFocus();
        if (target !== null) return target;
    }
    if (typeof initialFocus === 'string' && !['first', 'last', 'popup'].includes(initialFocus)) {
...
const Popup = ({
    ...
    useEffect((): (() => void) | undefined => {
        ...
        const anchorTrigger = 'trigger' in anchor ? resolveTrigger(anchor.trigger) : null;
    ...
    useEffect((): (() => void) | undefined => {
        ...
        const onPointerDown = (event: PointerEvent): void => {
            ...
            const trigger = 'trigger' in anchor ? resolveTrigger(anchor.trigger) : null;
...
export const PopupTrigger = forwardRef<HTMLButtonElement, PopupTriggerProps>(function PopupTrigger(
    { children, className, expanded, onKeyDown, onOpen, ...rest }: PopupTriggerProps,
    ref,
): React.JSX.Element {
    return (
        <button
...
export const PopupGroupLabel: React.FC<React.PropsWithChildren> = ({
    children,
}: React.PropsWithChildren): React.JSX.Element => <div className={styles.groupLabel}>{children}</div>;
