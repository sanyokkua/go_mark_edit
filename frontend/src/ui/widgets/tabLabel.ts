...
export function escapeUnsafeText(value: string): string {
    ...
    for (const character of value) {
        ...
        if (codePoint <= 0x1f || codePoint === 0x7f || BIDI_CONTROLS.has(codePoint)) {
...
function shortestUniqueSuffix(document: DocumentMetadata, matching: readonly DocumentMetadata[]): string | undefined {
    ...
    const ownIndex = matching.findIndex((candidate) => candidate.documentId === document.documentId);
...
export function tabLabelFor(document: DocumentMetadata, documents: readonly DocumentMetadata[]): TabLabel {
    ...
    const label = safeSuffix === undefined ? safeBasename : `${safeBasename} — ${safeSuffix}`;
    return {
 ...
 */
export function truncatedTabLabelParts(label: TabLabel, maxLength: number): TruncatedTabLabel {
...
export function truncateTabLabel(label: TabLabel, maxLength: number): string {
    ...
    const baseBudget = Math.max(1, maxLength - suffixBudget - separator.length - 1);
...
export function tabLabelsFor(documents: readonly DocumentMetadata[]): Map<string, TabLabel> {
    return new Map(documents.map((document) => [document.documentId, tabLabelFor(document, documents)]));
}
