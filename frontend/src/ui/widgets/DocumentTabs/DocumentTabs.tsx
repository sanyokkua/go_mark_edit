...
const DocumentTabs: React.FC<DocumentTabsProps> = ({
    ...
    const actionProjection = useMemo<ProjectedActionState>(
        () => ({
            ...
            orderedDocumentIds: orderedDocuments.map((document) => document.documentId),
        }),
    ...
    const labels = useMemo(() => tabLabelsFor(orderedDocuments), [orderedDocuments]);
    const contextDocument = orderedDocuments.find((document) => document.documentId === contextDocumentId);
    const contextIndex =
        contextDocument === undefined
            ? -1
            : orderedDocuments.findIndex((document) => document.documentId === contextDocument.documentId);
    ...
    const focusFallback = useCallback((): void => {
        ...
        const stripNewControl = document.querySelector<HTMLButtonElement>('[data-tab-new="true"]');
        if (stripNewControl !== null) {
            stripNewControl.focus();
            return;
        }
        document.querySelector<HTMLButtonElement>('[data-launcher-new="true"]')?.focus();
    ...
    const handleTabReorder = useCallback(
        async (documentId: string, targetIndex: number): Promise<void> => {
            const fromIndex = orderedDocuments.findIndex((document) => document.documentId === documentId);
            ...
            while (index !== targetIndex) {
                ...
                const result = await adapter.reorderDocument?.(documentId, next, revision);
                if (result?.error !== undefined) {
                    reportClassifiedError(dispatch, result.error, 'The tab could not be moved.', {
        ...
        async (documentId: string): Promise<unknown> => {
            try {
                ...
                const refusal = await flushOutgoingDocument(
                    ...
                    activeDocumentId === null || activeDocumentId === documentId ? undefined : activeDocumentId,
                );
    ...
    const activateDocument = useCallback(
        async (documentId: string): Promise<unknown> => {
            ...
            if (result !== undefined && 'error' in result && result.error !== undefined) {
                 ...
                 */
                reportClassifiedError(dispatch, result.error, 'The document could not be activated.', {
    ...
    const closeDocument = useCallback(
        async (
             ...
             */
            if (onCloseDocument) {
                return await onCloseDocument(documentId, expectedRevision, kind, targetDocumentIds);
            }
            const result = await adapter.closeDocument?.(documentId, expectedRevision);
            if (result?.error !== undefined) {
                reportClassifiedError(dispatch, result.error, 'The document could not be closed.');
            }
            return result;
        },
    ...
    const closeTargetedTab = useCallback(
        (documentId: string): void => {
            void closeDocument(documentId, tabSetRevision, 'single', [documentId]).then((result): void => {
                if (result?.activeDocumentId !== undefined && result.activeDocumentId !== '') {
    ...
    const moveTab = useCallback(
        async (
            ...
            const result = await adapter.reorderDocument?.(document.documentId, targetIndex, expectedRevision);
            if (result?.error !== undefined) {
                reportClassifiedError(dispatch, result.error, 'The tab could not be moved.', {
                    intent: 'reorder-document',
                    retry: {
                ...
                announce(
                    t('editor.tab.moved', {
                        filename: announcementName(labels.get(document.documentId), document.title),
    ...
    useEffect(() => {
        ...
        remediationSlotRef.current = async (remediation, freshTabSetRevision) => {
            ...
            const result = await moveTab(document, move.targetIndex, freshTabSetRevision);
    ...
    const handleTabAction = useCallback(
        async (action: TabContextAction, document: DocumentMetadata, targetIndex?: number): Promise<unknown> => {
            ...
            if (action === 'copy-path') {
                ...
                if (result?.error !== undefined) {
                    ...
                    reportClassifiedError(dispatch, result.error, 'The path could not be copied.', {
                        intent: 'copy-path',
                    });
            ...
            if (action === 'reveal-in-file-manager') {
                ...
                if (result?.error !== undefined) {
                    ...
                    reportClassifiedError(dispatch, result.error, 'The file manager could not reveal the document.', {
                        intent: 'reveal',
                    });
                }
                return result;
            ...
            if (action === 'close-tab') {
                return closeDocument(document.documentId, tabSetRevision, 'single', [document.documentId]);
            }
            const targets =
                action === 'close-right'
                    ? orderedDocuments.slice(
                          orderedDocuments.findIndex((candidate) => candidate.documentId === document.documentId) + 1,
                      )
                    : orderedDocuments.filter((candidate) => candidate.documentId !== document.documentId);
        ...
        [adapter, announce, closeDocument, dispatch, labels, moveTab, orderedDocuments, tabSetRevision],
    );
    ...
    useEffect((): (() => void) => {
        const navigate = (event: KeyboardEvent): void => {
             ...
             */
            const action = [...actionsForSurface('shortcuts'), ...actionsForSurface('tab-context')].find(
                (candidate) => candidate.shortcut === binding || candidate.shortcutAliases?.includes(binding),
            );
            if (action === undefined) return;
            const moving = action.id === 'move-tab-left' || action.id === 'move-tab-right';
            if (action.id !== 'next-tab' && action.id !== 'previous-tab' && !moving) {
                return;
            }
            if (orderedDocuments.length < 2 || activeDocumentId === null) return;
            const current = orderedDocuments.findIndex((document) => document.documentId === activeDocumentId);
            if (current < 0) return;
            if (moving) {
            ...
            const target = orderedDocuments[(current + offset + orderedDocuments.length) % orderedDocuments.length];
    ...
    }, [activateDocument, activeDocumentId, actionProjection, handleTabAction, modalOpen, orderedDocuments]);
            ...
            <TabBar
                ...
                tabs={orderedDocuments.map((document) => {
                    ...
                    return {
                        ...
                        readOnly: document.capability !== undefined && document.capability !== 'writable',
            ...
            {contextDocument !== undefined && contextIndex >= 0 ? (
                <TabContextMenu
                    ...
                    onClose={(options?: TabContextCloseOptions): void => {
                        ...
                        cancelPendingFocusRestore.current = whenApplicationRegainsForegroundFocus((): void => {
                            cancelPendingFocusRestore.current = null;
                            restoreMenuFocus(documentId);
                        });
                    }}
