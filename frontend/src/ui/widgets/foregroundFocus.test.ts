...
it('T142 runs the restoration on the window focus event, not before it', () => {
 ...
 */
// Proves: FR-FT-037 (the deferred-focus-restoration clause only).
it('T142 restores anyway once it can see the foreground was never given away', () => {
  const restore = jest.fn();
...
// Proves: FR-FT-037 (the deferred-focus-restoration clause only).
it('T142 keeps waiting past the grace check while another application holds the foreground', () => {
  const restore = jest.fn();
...
// Proves: FR-FT-037 (the deferred-focus-restoration clause only).
it('T142 detaches the listener when the wait is cancelled, so nothing accumulates', () => {
  const restore = jest.fn();
