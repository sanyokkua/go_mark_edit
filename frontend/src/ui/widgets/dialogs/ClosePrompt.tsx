...
import { t } from '../../../i18n';
import type { CloseChoice, ClosePlanSummary } from '../../../logic/store/appModelTypes';
...
const ClosePrompt: React.FC<ClosePromptProps> = ({
     ...
     */
    const title =
        plan.kind === 'quit' ? t('close.quit.title') : t(isSingle ? 'close.single.title' : 'close.multi.title');
    ...
    return (
        <ModalShell
            ...
            title={title}
        >
            <div className={styles.promptBody} data-close-kind={isSingle ? 'single' : plan.kind} data-close-prompt>
                ...
                <ul aria-label={t('close.dirtyTargets')}>
                    {dirtyTargets.map((target) => (
                        <li key={target.documentId} data-close-target={target.documentId}>
                            ...
                            {target.path !== undefined && target.path.length > 0 ? ` — ${target.path}` : ''}
                        </li>
                    ))}
                </ul>
