...
const StartupFailure: React.FC<StartupFailureProps> = ({
    ...
    <section className={styles.failure}>
        <div aria-busy={isRetrying} aria-label={t('startup.failure.message')} className={styles.panel} role="status">
            <h1>{t('startup.failure.title')}</h1>
            <p className={styles.message}>{failure === null ? t('startup.failure.message') : messageFor(failure)}</p>
            <div className={styles.actions}>
                {failure?.step !== 'bridge' ? (
                    <Button className={styles.retry} disabled={isRetrying} variant="primary" onClick={onRetry}>
                        {t('startup.retry')}
                    </Button>
                ) : failure === null ? (
                    <Button className={styles.retry} disabled={isRetrying} variant="primary" onClick={onRetry}>
