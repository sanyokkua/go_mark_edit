...
export interface UseCommandsResult {
    beginWrite: (kind: 'save' | 'save-as', targetDocumentId?: string) => Promise<WriteResult | undefined>;
    finishWrite: (
        kind: 'save' | 'save-as',
        documentId: string,
        contentRevision: number,
    ...
    onNewDocument: (expectedTabSetRevision: number) => Promise<EntryCommandOutcome | undefined>;
    onOpenDocument: (expectedTabSetRevision: number) => Promise<EntryCommandOutcome | undefined>;
    onOpenRecentFile: (path: string, expectedTabSetRevision: number) => Promise<EntryCommandOutcome | undefined>;
    onReopenLastFile: (expectedTabSetRevision: number) => Promise<EntryCommandOutcome | undefined>;
