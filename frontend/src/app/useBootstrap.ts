...
import { bootstrapSettingsProjection, disposeSettingsProjection } from '../logic/store/settingsProjection';
...
export interface UseBootstrapOptions {
    bootstrapModel?: (adapter: AppModelAdapter) => Promise<AppModelBootstrapResult>;
    bootstrapSettings?: (adapter: SettingsAdapter) => Promise<void>;
    loadAdapters?: () => Promise<BootstrapAdapters>;
    onReady?: (result: Extract<AppModelBootstrapResult, { status: 'ready' }>) => void;
}
...
const STARTUP_STEPS: readonly BootstrapStep[] = ['bridge', 'model', 'settings', 'window-ready'];
...
const defaultLoadAdapters = async (): Promise<BootstrapAdapters> => import('../logic/adapter');
...
function stepError(error: unknown, currentStep: BootstrapStep): StartupStepError {
    if (error instanceof StartupStepError) {
        return error;
    }
...
function withTimeout<T>(operation: () => Promise<T>, step: BootstrapStep): Promise<T> {
...
export function useBootstrap(options: UseBootstrapOptions = {}): BootstrapController {
    ...
    const bootstrapSettings = options.bootstrapSettings ?? bootstrapSettingsProjection;
    ...
    const modelResultRef = useRef<Extract<AppModelBootstrapResult, { status: 'ready' }> | null>(null);
            ...
            void (async (): Promise<void> => {
                ...
                try {
                    ...
                    if (isRetry && retryStep !== undefined && retryStep !== 'bridge') {
                        ...
                        await withTimeout(services.applicationAdapter.retryStartup, currentStep);
                    }

                    const firstStepIndex = retryStep === undefined ? 0 : STARTUP_STEPS.indexOf(retryStep);
                    if (firstStepIndex <= STARTUP_STEPS.indexOf('model') || modelResultRef.current === null) {
                        currentStep = 'model';
                        const modelResult = await withTimeout(
                            (): Promise<AppModelBootstrapResult> => bootstrapModel(services.appModelAdapter),
                            currentStep,
                        );
