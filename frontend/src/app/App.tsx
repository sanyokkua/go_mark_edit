...
import { ApplicationMenuRequestContext, type ApplicationMenuTarget } from '../ui/widgets/applicationMenuRequest';
import { EditorSessionProvider, useGuardedActivation } from '../ui/widgets/editorSession';
import { TabRemediationContext, type TabRemediationExecutor } from '../ui/widgets/tabRemediation';
...
import ExternalChangePrompt, { type ExternalChangeDecision } from '../ui/widgets/dialogs/ExternalChangePrompt';
...
const AppContents: React.FC = (): React.JSX.Element => {
     ...
     */
    const [activeBuffer, replaceActiveBuffer] = useState<ActiveBuffer | null>(null);
    const [remediationAnnouncement, setRemediationAnnouncement] = useState('');
    const [hydratedPendingCloseId, setHydratedPendingCloseId] = useState<string | null>(null);
    const [settingsOpen, setSettingsOpen] = useState(false);
    const [requestedApplicationMenu, setRequestedApplicationMenu] = useState<ApplicationMenuTarget | null>(null);
    const [aboutOpen, setAboutOpen] = useState(false);
    const [shortcutsOpen, setShortcutsOpen] = useState(false);
    const [normalization, setNormalization] = useState<NormalizationRequest | null>(null);
    const [externalConflict, setExternalConflict] = useState<ExternalConflictRequest | null>(null);
    const [tabExternalConflict, setTabExternalConflict] = useState<ConflictPreview | null>(null);
    ...
    const shutdownControllerRef = useRef<ShutdownController | undefined>(undefined);
    ...
    useEffect((): (() => void) => onApplicationForeground(runForegroundChecks), [runForegroundChecks]);
    ...
    const installExternalReload = useCallback(
        (acknowledgement: ActiveBuffer | undefined): void => {
            ...
            activation.acknowledge(generation, acknowledgement, acknowledgement.documentId);
    ...
    const tabExternalConflictValid =
        ...
        documentsById[tabExternalConflict.documentId]?.contentRevision === undefined ||
        documentsById[tabExternalConflict.documentId]?.contentRevision === tabExternalConflict.contentRevision;
    const onTabExternalConflictDecision = useCallback(
        async (decision: ExternalChangeDecision): Promise<void> => {
            ...
            if (result.error !== undefined) {
                reportClassifiedError(dispatch, result.error, 'The external-change decision could not be completed.');
                if (result.preview !== undefined) setTabExternalConflict(result.preview);
                return;
            }
        ...
        [dispatch, installExternalReload, tabExternalConflict, tabExternalConflictValid],
    );
    const closeRequestRef = useRef<{ kind: ClosePlanKind; targetDocumentIds: string[] } | undefined>(undefined);
    /*
    ...
    const reportClosePlanError = useCallback(
        (error: ClassifiedError | undefined): void => {
            ...
            reportClassifiedError(dispatch, error, t('notification.error.io.title'), options);
        },
        [dispatch],
    );
    /*
    ...
    const reportNativeCloseError = useCallback(
        (error: unknown): void => {
            ...
            dispatch(
                notifyError({ ...parsed, code: 'io', details: { subject: 'native-close' } }, 'native-close:io-failure'),
            );
        },
        [dispatch],
    );
    ...
    const completeClosePlan = useCallback(
        async (planId: string): Promise<TabTransitionResult> => {
            ...
            if (result.error !== undefined) {
                ...
                replaceActiveBuffer(result.activeBuffer);
            } else if (result.activeDocumentId === undefined || result.activeDocumentId === '') {
                replaceActiveBuffer(null);
            }
            if (result.error === undefined && result.orderedDocumentIds.length === 0) {
            ...
            if (isNativeClose && result.error === undefined) {
                try {
                    ...
                    if (refusal === undefined) {
                         ...
                         * close.
                         */
                        reportClassifiedError(dispatch, refusal, t('notification.error.io.title'), { intent: 'quit' });
                        await cancelNativeClose();
                    }
                } catch (error) {
                    reportNativeCloseError(error);
        ...
        [cancelNativeClose, clearCloseState, dispatch, reportClosePlanError, reportNativeCloseError],
    );
    const processClosePlanResult = useCallback(
        async (result: ClosePlanResult): Promise<TabTransitionResult | undefined> => {
            if (result.error !== undefined) {
                clearCloseState();
                reportClosePlanError(result.error);
                await cancelNativeClose();
            ...
            if (summary.status === 'cancelled' || summary.status === 'failed' || summary.status === 'complete') {
                clearCloseState();
                await cancelNativeClose();
                return undefined;
            }
            const normalizationTarget = summary.targets.find(
                (target) => target.dirty && target.choice === 'save' && target.normalizationToken !== undefined,
            );
            if (summary.status === 'ready' && normalizationTarget === undefined) {
                return completeClosePlan(summary.id);
            }

            if (nativeClosePendingRef.current && recoveryQuitConfirmedRef.current && recoverySurface !== null) {
                const discarded = await closePlanAdapter.resolveClosePlan(summary.id, [{ choice: 'discard-all' }]);
            ...
            if (normalizationTarget !== undefined) {
                ...
                setCloseNormalization({
                    ...
                    filename: normalizationTarget.displayName ?? normalizationTarget.title,
                });
                return undefined;
            }
            ...
            setCloseNormalization(null);
            if (conflictTarget !== undefined && conflictTarget.conflict !== undefined) {
                setCloseConflict({
                    planId: summary.id,
        ...
        [cancelNativeClose, clearCloseState, completeClosePlan, recoverySurface, reportClosePlanError],
    );
    const resolvePreparedClosePlan = useCallback(
        async (prepared: ClosePlanResult): Promise<TabTransitionResult | undefined> => {
            if (prepared.data?.status !== 'ready') {
                return processClosePlanResult(prepared);
            }
            const resolved = await closePlanAdapter.resolveClosePlan(prepared.data.id, []);
    ...
    const prepareNativeClosePlan = useCallback(async (): Promise<void> => {
        try {
            ...
            if (recoverySurface === null && state.activeBuffer?.documentId !== undefined) {
                await appModelAdapter.flushActiveSession?.(state.activeBuffer.documentId);
            }
            const targets = state.snapshot.orderedDocumentIds ?? orderedDocumentIds;
            const prepared = await closePlanAdapter.prepareClose('quit', targets, state.snapshot.tabSetRevision ?? 0);
            await resolvePreparedClosePlan(prepared);
        } catch (error) {
            reportNativeCloseError(error);
            await cancelNativeClose();
        }
    }, [cancelNativeClose, orderedDocumentIds, recoverySurface, reportNativeCloseError, resolvePreparedClosePlan]);
        ...
        async (choice: CloseChoice): Promise<void> => {
            ...
            const decisions: ClosePlanDecision[] =
                plan.kind === 'single' && dirtyTargets.length === 1
                    ...
                    : [{ choice }];
            const result = await closePlanAdapter.resolveClosePlan(plan.id, decisions);
    ...
    const onCloseNormalizationDecision = useCallback(
        async (confirm: boolean): Promise<void> => {
            ...
            const result = await closePlanAdapter.resolveClosePlan(requirement.planId, decisions);
            await processClosePlanResult(result);
        },
    ...
    const closeConflictValid =
        closeConflict === null ||
        closePlan?.targets.find((target) => target.documentId === closeConflict.documentId)?.contentRevision ===
            closeConflict.preview.contentRevision;
    const onCloseConflictDecision = useCallback(
        async (decision: ExternalChangeDecision): Promise<void> => {
            ...
            if (decision === 'reload') {
                await closePlanAdapter.resolveClosePlan(plan.id, [{ choice: 'cancel' }]);
                ...
                if (result.activeBuffer !== undefined) replaceActiveBuffer(result.activeBuffer);
                clearCloseState();
                const state = await appModelAdapter.getState();
            ...
            const cancelled = await closePlanAdapter.resolveClosePlan(plan.id, [{ choice: 'cancel' }]);
    ...
    const onCloseDocument = useCallback(
        async (
            ...
            const targets = targetDocumentIds.length > 0 ? targetDocumentIds : [documentId];
            ...
            if (activeBuffer?.documentId !== undefined && targets.includes(activeBuffer.documentId)) {
                await appModelAdapter.flushActiveSession?.(activeBuffer.documentId);
            }
            const prepared = await closePlanAdapter.prepareClose(kind, targets, expectedTabSetRevision);
    ...
    const onRemediate = useCallback(
        async (remediation: NotificationRemediation, notificationId: number, safeSubject: string): Promise<void> => {
            switch (remediation.intent) {
                ...
                case 'save-as': {
                     ...
                     */
                    const result = await beginWrite(remediation.intent, remediation.documentId);
        ...
        [announceRemediation, beginWrite, dispatch, onCloseDocument, retryEntryCommand],
    );
    ...
    const onExternalConflictDecision = useCallback(
            ...
            if (decision === 'reload') {
                ...
                const acknowledgement =
                    refreshedState.activeBuffer?.documentId === current.documentId
                        ...
                        : result.activeBuffer;
                activation.acknowledge(reloadGeneration, acknowledgement, current.documentId);
                /*
        ...
        [activation, externalConflict, externalConflictValid, finishWrite, reportWriteError],
    );
    ...
    const applicationMenuState = useMemo<ApplicationMenuState>(
        () => ({
            ...
            writable:
                ...
                (activeDocument.capability === undefined || activeDocument.capability === 'writable'),
    ...
    useEffect((): (() => void) | undefined => {
        ...
        const reportNativeGeometry = (): void => {
            void import('../logic/adapter').then(async ({ appModelAdapter, windowAdapter }): Promise<void> => {
    ...
    return (
        <ModalStateProvider modalOpen={modalOpen}>
            <TabRemediationContext.Provider value={tabRemediationRef}>
                ...
                >
                    <ApplicationMenuRequestContext.Provider value={setRequestedApplicationMenu}>
                        <div className="application-frame">
                            ...
                            >
                                ...
                                <AppearanceControlsContent visible={bootstrapStatus === 'ready'} />
                            </AppearanceSettingsProvider>
                                    ...
                                    <>
                                        {recoverySurface !== null ? (
                                            <section aria-label={t('recovery.title')} role="alert">
                                                ...
                                                <Button variant="secondary" onClick={requestRecoveryQuit}>
                                        ...
                                        <AppShell
                                            ...
                                            onOpenRecentFile={(path, expectedTabSetRevision): Promise<unknown> =>
                            ...
                            <ExternalChangePrompt
                                ...
                                open={bootstrapStatus === 'ready' && tabExternalConflict !== null}
                            ...
                            >
                                ...
                                {recoverySurface?.message !== undefined ? <p>{recoverySurface.message}</p> : null}
