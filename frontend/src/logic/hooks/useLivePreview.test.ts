...
import {
  ...
  type AcceptedBuffer,
} from '../adapter/appModelAdapter';
import { store } from '../store';
import { dismissNotification } from '../store/notificationsSlice';
import type { AppModelState } from '../store/appModelTypes';
import type { WireError } from '../utils/parseError';
import { type LivePreviewAdapter, useLivePreview } from './useLivePreview';
...
it('STORY-017-AC-2 keeps the last accepted preview on sync failure', async () => {
  jest.useFakeTimers();
  const wireError = {
    code: 'internal',
...
it('STORY-017-AC-3 (EC-RENDER-4) bounds large-document preview work', async () => {
  jest.useFakeTimers();
...
it('STORY-017-AC-6 ignores stale accepted generations', () => {
