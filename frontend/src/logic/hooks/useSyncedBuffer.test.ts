...
/*
 * T191. Monaco is seeded once per editor session: `CodeEditor` passes the
 ...
 * content changes.
 */
// Proves: FR-FT-030 — a reload starts a new editor session so the reloaded text
// is installed, without restarting the session for ordinary edits.
it('T191 starts a new editor session for an external replacement only', () => {
  const adapter = createAppModelAdapter(
    {
      getState: async () => ({
        data: {
