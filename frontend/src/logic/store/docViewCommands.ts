...
import type { DocViewInput, DocumentView, ViewArrangement } from './appModelTypes';

function docViewInputForArrangement(view: DocumentView, arrangement: ViewArrangement): DocViewInput {
    return {
...
export const setViewArrangement = createAsyncThunk<void, ViewArrangement, { state: RootState; rejectValue: WireError }>(
    'documents/setViewArrangement',
    async (arrangement, thunkApi) => {
...
export const setEditorPaneVisible = createAsyncThunk<void, boolean, { state: RootState; rejectValue: WireError }>(
    'documents/setEditorPaneVisible',
    async (editorVisible, thunkApi) => {
        const state = thunkApi.getState();
        const documentId = state.documents.activeDocumentId;
        if (documentId === null) {
            return thunkApi.rejectWithValue(missingDocument());
        ...
        try {
            ...
            await appModelAdapter.setDocView(
                ...
                docViewInputForPaneVisibility(document.view, editorVisible, document.view.previewVisible),
            );
        } catch (error) {
            return thunkApi.rejectWithValue(parseError(error));
        }
...
export const setPreviewPaneVisible = createAsyncThunk<void, boolean, { state: RootState; rejectValue: WireError }>(
    'documents/setPreviewPaneVisible',
    async (previewVisible, thunkApi) => {
        const state = thunkApi.getState();
        const documentId = state.documents.activeDocumentId;
        if (documentId === null) {
            return thunkApi.rejectWithValue(missingDocument());
        ...
        try {
            await appModelAdapter.setDocView(
                ...
                docViewInputForPaneVisibility(document.view, document.view.editorVisible, previewVisible),
            );
        } catch (error) {
            return thunkApi.rejectWithValue(parseError(error));
        }
