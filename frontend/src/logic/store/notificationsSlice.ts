...
function classifiedSubject(error: WireError): string {
    return (
        error.details?.subject ?? error.details?.operation ?? error.details?.field ?? error.details?.name ?? error.code
    );
}

function toNotification(input: NotificationInput, error?: WireError): Notification {
...
function findDuplicate(notifications: Notification[], incoming: Notification): Notification | undefined {
    return notifications.find(
        (notification) => notification.code === incoming.code && notification.subject === incoming.subject,
    );
}
...
function mergeRemediations(
    ...
    for (const candidate of incoming) {
        if (offered.some((current) => current.action === candidate.action)) continue;
        offered.push(candidate);
    }
    return offered;
}

function refreshDuplicate(duplicate: Notification, incoming: Notification): void {
    duplicate.count += 1;
    duplicate.error = incoming.error;
    duplicate.message = incoming.message;
    duplicate.refreshGeneration += 1;
    duplicate.remediations = mergeRemediations(duplicate.remediations, incoming.remediations);
...
function enqueueToast(state: NotificationsState, incoming: Notification): void {
    const duplicate = findDuplicate(state.items, incoming) ?? findDuplicate(state.queuedErrors, incoming);
    ...
    const oldestNonError = state.items.findIndex((notification) => notification.severity !== 'error');
    if (oldestNonError >= 0) {
...
const notificationsSlice = createSlice({
    ...
    reducers: {
        notifyError: {
            prepare(error: WireError, subject = classifiedSubject(error)): { payload: Notification } {
        ...
        dismissNotification(state, action: PayloadAction<number>): void {
            const visibleIndex = state.items.findIndex((notification) => notification.id === action.payload);
            if (visibleIndex >= 0) {
            ...
            state.queuedErrors = state.queuedErrors.filter((notification) => notification.id !== action.payload);
        },
        dismissStuckCommand(state, action: PayloadAction<string>): void {
            const visibleIndex = state.items.findIndex((notification) => notification.requestId === action.payload);
            if (visibleIndex >= 0) {
                state.items.splice(visibleIndex, 1);
                const queuedError = state.queuedErrors.shift();
            ...
            state.queuedErrors = state.queuedErrors.filter((notification) => notification.requestId !== action.payload);
        },
        clearCondition(state, action: PayloadAction<number>): void {
            state.banners = state.banners.filter((notification) => notification.id !== action.payload);
        },
