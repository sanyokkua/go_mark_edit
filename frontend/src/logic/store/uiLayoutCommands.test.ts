 ...
 */
it('FR-WS-011 collapsing the divider to zero hides the workspace', async () => {
...
it('FR-WS-011 an ordinary drag width carries no visibility change', async () => {
  hydrateLayout({ sidebarVisible: true, sidebarWidth: 240 });
 ...
 */
it('FR-WS-011 showing a zero-width workspace restores the binding width', async () => {
...
it('FR-WS-011 showing a workspace that already has a width leaves it alone', async () => {
...
it('FR-WS-011 hiding the workspace never rewrites its width', async () => {
 ...
 */
it('FR-WS-011 showing a never-sized workspace issues no width', async () => {
