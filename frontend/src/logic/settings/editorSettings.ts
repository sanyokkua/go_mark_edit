...
export function useEditorSettings(): EditorSettingsState {
    ...
    const commands = useMemo(() => createSettingsCommandOwner(settingsAdapter), []);
    ...
    const updateMarkdown = useCallback(
        (patch: Partial<MarkdownSettings>): Promise<void> => commands.updateMarkdown(markdownSettings, patch, dispatch),
