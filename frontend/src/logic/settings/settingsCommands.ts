import type { AppearanceSettings, EditorSettings, FileSettings, MarkdownSettings } from '../adapter/settingsTypes';
...
type SettingsCommandAdapter = Pick<
    SettingsAdapter,
    'resetAppearance' | 'updateAppearance' | 'updateEditor' | 'updateFile' | 'updateMarkdown'
>;
...
export interface SettingsCommandOwner {
    resetAppearance: (acknowledge: SettingsAcknowledgement<AppearanceSettings>) => Promise<void>;
    updateAppearance: (
        current: AppearanceSettings,
        patch: Partial<AppearanceSettings>,
        acknowledge: SettingsAcknowledgement<AppearanceSettings>,
...
export function createSettingsCommandOwner(adapter: SettingsCommandAdapter): SettingsCommandOwner {
    ...
    return {
        resetAppearance: (acknowledge): Promise<void> => enqueue(() => resetAppearanceSettings(adapter, acknowledge)),
        updateAppearance: (current, patch, acknowledge): Promise<void> =>
            enqueue(() => acknowledgeAppearanceSettingsUpdate(adapter, current, patch, acknowledge)),
        updateEditor: (current, patch, dispatch): Promise<void> =>
            enqueue(() => acknowledgeEditorSettingsUpdate(adapter, current, patch, dispatch)),
