...
function invocationRecord(value: unknown): InvocationRecord | undefined {
    return typeof value === 'object' && value !== null ? (value as InvocationRecord) : undefined;
}
...
function projectedDocumentIsWritable(context: ActionDispatchContext): boolean {
    ...
    return documentId !== undefined && projected?.documents?.[documentId]?.capability === 'writable';
}
...
export async function dispatchAction(actionId: ActionId, context: ActionDispatchContext = {}): Promise<ActionResult> {
    ...
    return actionResultFromInvocation(actionId, context.documentId, context.sessionDocumentId, invocation);
}
