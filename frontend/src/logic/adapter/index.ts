...
import { dismissStuckCommand, notifyStuckCommand } from '../store/notificationsSlice';
import { createCommandInvoker, getBootstrapStatus, type CommandBindOptions } from './command';
import { EVENTS } from './events';

const commandInvoker = createCommandInvoker({
    isReady: (): boolean => getBootstrapStatus() === 'ready',
    requestFactory: (requestId): bridge.Request => new bridge.Request({ id: requestId }),
    noticeOwner: {
...
function normalizeSaveStatus(status: string | undefined): DocumentMetadata['status'] {
    switch (status) {
        case 'not-saved':
...
function normalizeConflictResult(result: apperr.ConflictResult): ConflictResult {
    return {
        activeBuffer:
...
function normalizeTabTransitionResult(result: apperr.TabTransitionResult): TabTransitionResult {
    return {
...
function normalizePathCommandResult(result: apperr.PathCommandResult): PathCommandResult {
    return {
        ...
        data:
            result.data === undefined
                ...
                : {
                      ...
                      committedProjectionRevision: result.data.committedProjectionRevision,
                      targetPath: result.data.targetPath,
                      targetPathAdopted: result.data.targetPathAdopted,
                      lineEndingOutcome: result.data.lineEndingOutcome as LineEndingOutcome,
                      bomOutcome: result.data.bomOutcome as 'preserved' | 'absent',
                      resyncRequired: result.data.resyncRequired,
                  },
        decisionToken: result.decisionToken,
        proposedEnding: result.proposedEnding === undefined ? undefined : (result.proposedEnding as 'lf' | 'crlf'),
        documentRevision: result.documentRevision,
        conflict: normalizeConflictPreview(result.conflict),
        error: normalizeClassifiedError(result.error),
    };
...
const generatedSettingsBindings: SettingsBindings = {
    ...
    updateAppearance: command('SettingsHandler.UpdateAppearance', UpdateAppearance),
    resetAppearance: command('SettingsHandler.ResetAppearance', ResetAppearance),
    updateContentPrivacy: command('SettingsHandler.UpdateContentPrivacy', UpdateContentPrivacy),
...
const commandInvokerSave = command('AppModelHandler.Save', Save, {
    pacing: (documentId: string): 'bounded' | 'user-paced' =>
        store.getState().documents.byId[documentId]?.path === '' ? 'user-paced' : 'bounded',
});
const commandInvokerSaveAs = command('AppModelHandler.SaveAs', SaveAs, {
    pacing: 'user-paced',
});
const commandInvokerCancelNormalization = command('AppModelHandler.CancelNormalization', CancelNormalization);
const commandInvokerCheckExternalChanges = command('AppModelHandler.CheckExternalChanges', CheckExternalChanges);
const commandInvokerReloadFromDisk = command('AppModelHandler.ReloadFromDisk', ReloadFromDisk);
const commandInvokerAuthorizeKeepMine = command('AppModelHandler.AuthorizeKeepMine', AuthorizeKeepMine);
const commandInvokerSkipConflict = command('AppModelHandler.SkipConflict', SkipConflict);
const commandInvokerCancelConflict = command('AppModelHandler.CancelConflict', CancelConflict);
...
const commandInvokerReorderDocument = command('AppModelHandler.ReorderDocument', ReorderDocument);
const commandInvokerCloseDocument = command('AppModelHandler.CloseDocument', CloseDocument);
const commandInvokerCopyPath = command('AppModelHandler.CopyPath', CopyPath);
const commandInvokerRevealInFileManager = command('AppModelHandler.RevealInFileManager', RevealInFileManager);
const commandInvokerUpdateBuffer = command('AppModelHandler.UpdateBuffer', UpdateBuffer);
const commandInvokerSetDocView = command('AppModelHandler.SetDocView', SetDocView);
const commandInvokerSetUILayout = command('AppModelHandler.SetUILayout', SetUILayout);
const commandInvokerPrepareClose = command('AppModelHandler.PrepareClose', PrepareClose);
const commandInvokerResolveClosePlan = command('AppModelHandler.ResolveClosePlan', ResolveClosePlan, {
    pacing: 'user-paced',
});
...
export const documentWriteAdapter = createDocumentWriteAdapter({
    save: async (documentId, contentRevision, decisionToken) =>
        normalizeWriteResult(await commandInvokerSave(documentId, contentRevision, decisionToken)),
    saveAs: async (documentId, contentRevision, decisionToken) =>
        normalizeWriteResult(await commandInvokerSaveAs(documentId, contentRevision, decisionToken)),
...
export const documentConflictAdapter = createDocumentConflictAdapter({
    authorizeKeepMine: async (documentId, contentRevision, path, detectedVersion) =>
    ...
    cancelConflict: async (documentId, contentRevision, detectedVersion) =>
        normalizeConflictResult(
            await commandInvokerCancelConflict(documentId, contentRevision, new apperr.DiskVersion(detectedVersion)),
        ),
    checkExternalChanges: async (documentId) =>
        normalizeConflictResult(await commandInvokerCheckExternalChanges(documentId)),
    reloadFromDisk: async (documentId, contentRevision, detectedVersion) =>
        normalizeConflictResult(
            await commandInvokerReloadFromDisk(documentId, contentRevision, new apperr.DiskVersion(detectedVersion)),
        ),
    skipConflict: async (documentId, contentRevision, detectedVersion) =>
        normalizeConflictResult(
            await commandInvokerSkipConflict(documentId, contentRevision, new apperr.DiskVersion(detectedVersion)),
        ),
});
...
const generatedAppModelBindings: AppModelBindings = {
    ...
    newDocument: async (expectedTabSetRevision) =>
        normalizeTransitionResult(await commandInvokerNewDocument(expectedTabSetRevision)),
    ...
    reopenLastFile: async (expectedTabSetRevision) =>
        normalizeOpenResult(await commandInvokerReopenLastFile(expectedTabSetRevision)),
    activateDocument: async (documentId, expectedTabSetRevision) =>
        normalizeTransitionResult(await commandInvokerActivateDocument(documentId, expectedTabSetRevision)),
    reorderDocument: async (documentId, targetIndex, expectedTabSetRevision) =>
        normalizeTabTransitionResult(
            await commandInvokerReorderDocument(documentId, targetIndex, expectedTabSetRevision),
        ),
    closeDocument: async (documentId, expectedTabSetRevision) =>
        normalizeTabTransitionResult(await commandInvokerCloseDocument(documentId, expectedTabSetRevision)),
    copyPath: async (documentId) => normalizePathCommandResult(await commandInvokerCopyPath(documentId)),
    revealInFileManager: async (documentId) =>
        normalizePathCommandResult(await commandInvokerRevealInFileManager(documentId)),
    updateBuffer: commandInvokerUpdateBuffer,
    setDocView: (documentId, view) => commandInvokerSetDocView(documentId, new apperr.DocViewInput(view)),
    setUILayout: (layout) => commandInvokerSetUILayout(new apperr.UILayout(layout)),
};
...
export const appModelAdapter = createAppModelAdapter(generatedAppModelBindings, wailsRuntime);
...
export const closePlanAdapter = createClosePlanAdapter({
    ...
    executeClosePlan: (planId) => commandInvokerExecuteClosePlan(planId).then(normalizeTabTransitionResult),
});
...
export const nativeLifecycleAdapter: NativeLifecycleAdapter = {
    onCloseRequested: (listener) =>
        EventsOn(EVENTS.applicationCloseRequested, (payload: unknown) => listener(closeRequestID(payload))),
    requestQuit: () => Quit(),
    authorizeQuit: async (closeID) => normalizeClassifiedError((await commandInvokerAuthorizeQuit(closeID)).error),
