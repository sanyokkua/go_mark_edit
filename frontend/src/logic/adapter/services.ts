...
export interface SettingsBindings {
    ...
    updateContentPrivacy: (settings: ContentPrivacySettings) => Promise<VoidResult>;
...
export interface DocumentWriteBindings {
    save: (documentId: string, contentRevision: number, decisionToken: string) => Promise<WriteResult>;
    saveAs: (documentId: string, contentRevision: number, decisionToken: string) => Promise<WriteResult>;
    /**
     ...
     * dismissed prompt.
     */
    cancelNormalization: (documentId: string, decisionToken: string) => Promise<{ error?: unknown }>;
}
...
export interface ClosePlanBindings {
    ...
    resolveClosePlan: (planId: string, decisions: ClosePlanDecision[]) => Promise<ClosePlanResult>;
            ...
            target.conflict === undefined
                ...
                : {
                      ...
                      detectedDiskVersion: {
                          ...
                          modifiedUnixNano: String(target.conflict.detectedDiskVersion.modifiedUnixNano),
                      },
                  },
    };
}
...
export function createClosePlanAdapter(bindings: ClosePlanBindings): ClosePlanAdapter {
    const prepareClose = guardArity('AppModelHandler.PrepareClose', bindings.prepareClose);
    const resolveClosePlan = guardArity('AppModelHandler.ResolveClosePlan', bindings.resolveClosePlan);
    const executeClosePlan = guardArity('AppModelHandler.ExecuteClosePlan', bindings.executeClosePlan);
    return {
        prepareClose: async (kind, targetDocumentIds, expectedTabSetRevision) =>
            normalizeClosePlanResult(await prepareClose(kind, targetDocumentIds, expectedTabSetRevision)),
...
export function createDocumentConflictAdapter(bindings: DocumentConflictBindings): DocumentConflictAdapter {
    const checkExternalChanges = guardArity('AppModelHandler.CheckExternalChanges', bindings.checkExternalChanges);
    const reloadFromDisk = guardArity('AppModelHandler.ReloadFromDisk', bindings.reloadFromDisk);
    const authorizeKeepMine = guardArity('AppModelHandler.AuthorizeKeepMine', bindings.authorizeKeepMine);
    const skipConflict = guardArity('AppModelHandler.SkipConflict', bindings.skipConflict);
    const cancelConflict = guardArity('AppModelHandler.CancelConflict', bindings.cancelConflict);
...
export function createDocumentLifecycleAdapter(bindings: DocumentLifecycleBindings): DocumentLifecycleAdapter {
    const newDocument = guardArity('AppModelHandler.NewDocument', bindings.newDocument);
    const openDocument = guardArity('AppModelHandler.OpenDocument', bindings.openDocument);
    ...
    return {
        newDocument: (expectedTabSetRevision: number) => newDocument(expectedTabSetRevision),
        openDocument: (expectedTabSetRevision: number) => openDocument(expectedTabSetRevision),
        openRecentFile:
            openRecentFile === undefined
                ? undefined
                : (path: string, expectedTabSetRevision: number) => openRecentFile(path, expectedTabSetRevision),
        reopenLastFile:
            reopenLastFile === undefined
                ? undefined
                : (expectedTabSetRevision: number) => reopenLastFile(expectedTabSetRevision),
    };
}

export function createDocumentWriteAdapter(bindings: DocumentWriteBindings): DocumentWriteAdapter {
    const save = guardArity('AppModelHandler.Save', bindings.save);
    const saveAs = guardArity('AppModelHandler.SaveAs', bindings.saveAs);
    const cancelNormalization = guardArity('AppModelHandler.CancelNormalization', bindings.cancelNormalization);

    return {
        save: (documentId, contentRevision, decisionToken) => save(documentId, contentRevision, decisionToken),
        saveAs: (documentId, contentRevision, decisionToken) => saveAs(documentId, contentRevision, decisionToken),
        cancelNormalization: (documentId, decisionToken) => cancelNormalization(documentId, decisionToken),
    };
}

export function createSettingsAdapter(bindings: SettingsBindings): SettingsAdapter {
    const getSettings = guardArity('SettingsHandler.GetSettings', bindings.getSettings);
    const updateAppearance = guardArity('SettingsHandler.UpdateAppearance', bindings.updateAppearance);
    const resetAppearance = guardArity('SettingsHandler.ResetAppearance', bindings.resetAppearance);
    const updateContentPrivacy = guardArity('SettingsHandler.UpdateContentPrivacy', bindings.updateContentPrivacy);
    const updateMarkdown = guardArity('SettingsHandler.UpdateMarkdown', bindings.updateMarkdown);
    const updateEditor = guardArity('SettingsHandler.UpdateEditor', bindings.updateEditor);
    const updateFile = guardArity('SettingsHandler.UpdateFile', bindings.updateFile);
    ...
    return {
        ...
        async updateContentPrivacy(settings: ContentPrivacySettings): Promise<void> {
