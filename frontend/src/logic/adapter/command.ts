...
export function createCommandInvoker(options: CommandInvokerOptions): CommandInvoker {
    ...
    const requestFactory = options.requestFactory ?? ((id): { id: string } => ({ id }));
    const setTimer =
        options.setTimer ??
        ((callback: () => void, delay: number): TimerHandle => globalThis.setTimeout(callback, delay));
    const clearTimer = options.clearTimer ?? ((timer: TimerHandle): void => globalThis.clearTimeout(timer));
    const pending = new Map<string, PendingCommand<unknown[], unknown>>();

    function withdrawNotice<TResult>(entry: PendingCommand<unknown[], TResult>): void {
    ...
    function scheduleNotice<TResult>(entry: PendingCommand<unknown[], TResult>, pacing: CommandPacing): void {
        if (pacing !== 'bounded' || !options.isReady()) return;
        entry.timer = setTimer((): void => {
            entry.timer = undefined;
            if (entry.settled || entry.cancelled || entry.noticeShown || !options.isReady()) {
                return;
            }
    ...
    function bind<TArgs extends unknown[], TResult>(
        ...
        const expectedArity = Math.max(0, bindOptions.expectedArity ?? bound.length - 1);
        const pacing = bindOptions.pacing ?? 'bounded';

        const invoke = (...args: TArgs): Promise<TResult> => {
            if (args.length !== expectedArity) {
                return Promise.reject(
                    new Error(`${commandName} expects ${expectedArity} argument(s), received ${args.length}.`),
                );
            }

            const requestId = uuid();
            let resolvePromise: (value: TResult | PromiseLike<TResult>) => void = (): void => undefined;
            ...
            send(entry);
            scheduleNotice(entry, typeof pacing === 'function' ? pacing(...args) : pacing);
            return promise;
        };
