...
function blocks(css) {
    return [...css.matchAll(/([^{}]+)\{([^{}]*)\}/g)].map(([, sourceSelector, sourceDeclarations]) => {
        ...
        if (theme && !themes.includes(theme[1])) throw new Error(`Untraceable palette selector ${selector}`);
        if (mode && !modes.includes(mode[1])) throw new Error(`Untraceable palette selector ${selector}`);

        const declarations = {};
        for (const [, name, value] of sourceDeclarations.matchAll(/(--[\w-]+)\s*:\s*([^;]+);/g)) {
...
function resolvedTokens(css, theme, mode) {
    ...
    for (const token of required) {
        ...
        if (values[token].includes('var(')) throw new Error(`Unresolved token ${token} for ${theme}/${mode}`);
    }
    return values;
}
...
function monacoColor(value) {
    ...
    const hex = [red, green, blue].map((component) => Number(component).toString(16).padStart(2, '0')).join('');
...
async function generateFiles({ input, themesOutput, highlightOutput }) {
    ...
    const [themesConfig, highlightConfig] = await Promise.all([
        resolveConfig(resolve(__dirname, '../src/logic/theme/generatedEditorThemes.ts')),
        resolveConfig(resolve(__dirname, '../src/logic/theme/generatedHighlight.css')),
    ]);
