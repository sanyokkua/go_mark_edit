...
test('quitting from the pre-ready failure screen exits the application process', async ({ app }) => {
    await breakSettingsDatabase(app.profileDirectory);
    await app.launch();

    const failure = failureSurface(app.page);
    await expect(failure).toBeVisible();
    await expect(failure.getByRole('heading', { name: 'GoMarkEdit could not start' })).toBeVisible();
...
test('retrying a failed settings startup mounts the shell once and can quit cleanly', async ({ app }) => {
    ...
    await expect(failure).toBeVisible();
    await expect(failure).toContainText(/Settings:\s+\S/u);
    await expect(failure.getByRole('button', { name: 'Retry', exact: true })).toHaveCount(1);
    await expect(failure.getByRole('button', { name: 'Quit', exact: true })).toHaveCount(1);
...
test('a pre-ready quit request is discovered after startup recovery', async ({ app }) => {
    const settingsDatabase = settingsDatabaseDirectory(app.profileDirectory);
    await breakSettingsDatabase(app.profileDirectory);
    await app.launch();
