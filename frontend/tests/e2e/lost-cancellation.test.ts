...
test('cancelling a dirty quit leaves the next quit with a new close decision', async ({ app }) => {
    ...
    await expect
        .poll(() => app.page.locator('[data-editor-surface]').evaluate((element) => element.textContent ?? ''))
        .toContain('Changed');
