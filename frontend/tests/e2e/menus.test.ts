...
async function expectPopupContract(page: Page, popup: Locator, checkFocusRing: boolean): Promise<void> {
    ...
    const state = await popup.evaluate((element) => {
        ...
        const ringItems = items.filter((item) => getComputedStyle(item).boxShadow !== 'none');
        return {
...
async function setPalette(
    ...
    const settings = await openMenubarFamily(page, { label: 'Settings', popup: 'settings-menu' }, false);
    await settings.popup.getByRole('radio', { name: themeLabel, exact: true }).click();
    await settings.popup.getByRole('radio', { name: modeLabel, exact: true }).click();
