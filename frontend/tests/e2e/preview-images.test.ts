...
test('case 7 serves bounded in-folder images and keeps every other source inert', async ({ app }) => {
    const sourcePath = await app.writeDocument('D/a.md', '');
    await writeFile(join(app.documentDirectory, 'D', 'inside.png'), ONE_PIXEL_PNG);
    await writeFile(join(app.documentDirectory, 'outside.png'), ONE_PIXEL_PNG);
    await writeFile(join(app.documentDirectory, 'D', 'huge.png'), Buffer.alloc(21 * 1024 * 1024));
    ...
    await expect.poll(() => inside.evaluate((image) => (image as HTMLImageElement).naturalWidth)).toBeGreaterThan(0);
    ...
    await expect.poll(() => getState(page).then((state) => state.data?.activeBuffer?.content)).toBe(untitledSource);
