...
it('buffers a close event before readiness and handles it after readiness', async () => {
    ...
    const view = render(
        <Harness bootstrapStatus="loading" hydratedPendingCloseId={null} native={native} onRequest={onRequest} />,
    );
    ...
    view.rerender(
        <Harness bootstrapStatus="ready" hydratedPendingCloseId={null} native={native} onRequest={onRequest} />,
    );

    await waitFor(() => expect(onRequest).toHaveBeenCalledWith('early-close'));
    expect(screen.getByRole('status', { name: 'Pending close' })).toHaveTextContent('early-close');
});
...
it('handles a pending close discovered in the hydrated state', async () => {
    ...
    expect(screen.getByRole('status', { name: 'Pending close' })).toHaveTextContent('hydrated-close');
});
...
it('keeps the pending close visible until cancellation is acknowledged', async () => {
    ...
    const onRequest = jest.fn();
    render(<Harness bootstrapStatus="ready" hydratedPendingCloseId={null} native={native} onRequest={onRequest} />);

    act((): void => native.emit('cancel-close'));
    await waitFor(() => expect(onRequest).toHaveBeenCalledWith('cancel-close'));
    fireEvent.click(screen.getByRole('button', { name: 'Cancel' }));

    expect(screen.getByRole('status', { name: 'Pending close' })).toHaveTextContent('cancel-close');
