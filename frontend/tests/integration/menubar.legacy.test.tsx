...
it('renders File, Settings, View, About in binding order with exact deferred inventories', async () => {
    ...
    expect(screen.getByRole('menu', { name: 'File' })).toHaveAttribute('data-viewport-popup', 'file-menu');
    ...
    expect(screen.getByRole('menu', { name: 'About GoMarkEdit' })).toHaveAttribute('data-viewport-popup', 'about-menu');
...
it('keeps document identity inside the top menu row without a vertical identity block', () => {
    ...
    expect(within(menu).getByRole('heading', { name: 'projects / notes.md' })).toBeVisible();
...
it('routes the available File New/Open controls through the lifecycle dispatcher', async () => {
    ...
    expect(within(fileMenu).getByRole('menuitem', { name: 'New File' })).toBeEnabled();
    expect(within(fileMenu).getByRole('menuitem', { name: 'Open File' })).toBeEnabled();
    ...
    await waitFor(() =>
        expect(dispatch).toHaveBeenCalledWith('open-file', expect.objectContaining({ applicationFocused: true })),
    );
...
it('closes the narrow View menu after choosing an arrangement', async () => {
    ...
    await waitFor(() => expect(screen.queryByRole('menu', { name: 'View options' })).toBeNull());
});
...
it('places the functional sidebar and deferred Assistant controls at the menu-row edge', () => {
    ...
    expect(dispatch).toHaveBeenCalledWith('toggle-sidebar', expect.objectContaining({ invoke: expect.any(Function) }));
...
it('makes the visible View control the Radix menu trigger', () => {
    ...
    expect(screen.getByRole('button', { name: 'View' })).toHaveAttribute('data-view-trigger');
});
...
interface FileAcceleratorCase {
    ...
    readonly prop: 'onNewDocument' | 'onOpenDocument' | 'onSave' | 'onSaveAs' | 'onCloseDocument' | 'onReopenLastFile';
}
...
function renderMenuRowWithFileActions(
    ...
    render(
        <Menubar
            ...
            onAbout={jest.fn()}
            onCloseDocument={(shell.closeDocument ?? true) ? callbacks.onCloseDocument : undefined}
...
describe.each(fileAcceleratorCases)(
    ...
    ({ actionId, event, prop }: FileAcceleratorCase) => {
        it(`runs ${actionId} when the key is pressed at the window`, async () => {
            ...
            expect(dispatch).toHaveBeenCalledWith(actionId, expect.objectContaining({ invoke: expect.any(Function) }));
            /*
...
it('disables the File menu Close Tab row when no close callback is bound', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'File' }));
    const row = within(screen.getByRole('menu', { name: 'File' })).getByRole('menuitem', { name: 'Close Tab' });
...
it('leaves Mod+W unclaimed when no close callback is bound', () => {
    ...
    expect(fireEvent.keyDown(window, { code: 'KeyW', ctrlKey: true, key: 'w' })).toBe(true);
    expect(callbacks.onCloseDocument).not.toHaveBeenCalled();
});
...
it('closes the active document when the File menu Close Tab row is clicked', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'File' }));
    const fileMenu = screen.getByRole('menu', { name: 'File' });
    fireEvent.click(within(fileMenu).getByRole('menuitem', { name: 'Close Tab' }));

    await waitFor(() => expect(callbacks.onCloseDocument).toHaveBeenCalledTimes(1));
    expect(dispatch).toHaveBeenCalledWith('close-tab', expect.objectContaining({ documentId: 'document-1' }));
    dispatch.mockRestore();
});
