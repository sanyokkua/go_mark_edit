...
let mockStatePatchListener: ((patch: import('../../src/logic/store/appModelTypes').AppStatePatch) => void) | undefined;
...
jest.mock('../../src/logic/adapter', () => ({
    ...
    appModelAdapter: {
        ...
        subscribeStatePatches: jest.fn(
            (listener: (patch: import('../../src/logic/store/appModelTypes').AppStatePatch) => void): (() => void) => {
...
function resetMockMonaco(): void {
    ...
    mockRuntime.editor = {
        ...
        onDidChangeCursorSelection: jest.fn((listener: (event: editor.ICursorSelectionChangedEvent) => void) => {
        ...
        saveViewState: jest.fn((): editor.ICodeEditorViewState => {
            ...
            viewState.selection = mockRuntime.selection === null ? undefined : { ...mockRuntime.selection };
            return viewState;
        }),
    ...
    const MockMonacoEditor = (props: EditorProps): React.JSX.Element => {
        ...
        return React.createElement('textarea', {
            ...
            onChange: (event: React.ChangeEvent<HTMLTextAreaElement>): void => {
                ...
                props.onChange?.(event.target.value, {} as Parameters<NonNullable<EditorProps['onChange']>>[1]);
            },
        });
    };
...
import { bootstrapAppModelProjection, disposeAppModelProjection } from '../../src/logic/store/appModelProjection';
import type { AppModelState, DocumentMetadata } from '../../src/logic/store/appModelTypes';
...
async function renderStatusEditor(document: DocumentMetadata, content: string): Promise<void> {
    ...
    const { default: AppShell } = await import('../../src/ui/widgets/AppShell');
...
it('keeps a long document title available while both centre panes render', async () => {
    const title = 'A deliberately long Markdown document title that remains available in the editor pane';
    ...
    expect(screen.getByLabelText('Editor pane')).toHaveTextContent(`Editor · ${title}`);
...
function renderLivePreviewEditor(
    ...
    return render(
        <Provider store={store}>
            <EditorSessionContext.Provider value={{ documentId: document.documentId, content }}>
...
const CommittedWriteHarness: React.FC<{
    ...
    return (
        <>
            <button type="button" onClick={(): void => setContent('# After the committed write')}>
                Apply committed-write patch
            </button>
            <EditorSessionProvider activeBuffer={{ documentId: 'document-1', content }}>
                <EditorView adapter={adapter} />
            </EditorSessionProvider>
        </>
    );
...
it('renders the translated editor catalogue and preview text', async () => {
    await act(async (): Promise<void> => {
    ...
    expect(screen.getByRole('radiogroup', { name: 'View arrangement' })).toBeInTheDocument();
...
it('applies every persisted palette to the rendered Settings control', async () => {
    ...
    for (const [theme, mode] of palettes) {
        ...
        expect(rendered.getByRole('button', { name: 'Settings' })).toBeInTheDocument();
        rendered.unmount();
    }
...
it('keeps the editor activation token when a committed-write patch changes the buffer object', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Apply committed-write patch' }));
...
it('preserves replacement undo and UpdateBuffer routing', async () => {
    const document = statusDocument({
        view: {
            arrangement: 'editor',
            editorVisible: true,
    ...
    const updateBuffer = jest.fn<Promise<void>, [string, string]>(async (): Promise<void> => undefined);
    const adapter: EditorViewAdapter = {
        flushBuffer: async (): Promise<void> => undefined,
        flushDocView: async (): Promise<void> => undefined,
        subscribeAcceptedBuffers: (): (() => void) => (): void => undefined,
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Replace range through session' }));
    fireEvent.click(screen.getByRole('button', { name: 'Replace all through session' }));
...
it('displays live one-based cursor position without Redux truth', async () => {
...
it('renders higher-revision backend word count without Monaco or store content', async () => {
...
it('reflects a backend Preview-only view patch in the status bar', async () => {
    const document = statusDocument();
    ...
    expect(screen.getByLabelText('Editor pane')).not.toHaveAttribute('aria-hidden', 'true');
    expect(screen.getByLabelText('Preview pane')).toBeInTheDocument();
...
it('formats Phase-01 canonical wire metadata labels', async () => {
    await renderStatusEditor(statusDocument({ encoding: 'utf-8', lineEnding: 'lf' }), 'Untitled buffer');
...
it('keeps preview text outside Redux', () => {
    ...
    expect(JSON.stringify(store.getState())).not.toContain('Ignored after cleanup');
});

it('keeps the editor region in binding content order without an extra wrapper', async () => {
    ...
    const tabBar = editorRegion.querySelector(':scope > [data-bar-role="tablist-host"]');
    expect(tabBar).not.toBeNull();
    expect(tabBar?.querySelector(':scope [role="tablist"]')).not.toBeNull();
    expect(editorRegion.querySelector(':scope > [data-bar-role="toolbar"]')).not.toBeNull();
    expect(editorRegion.querySelector(':scope > [class*="stage"]')).not.toBeNull();
});

it('renders accepted GFM within the debounce target', async () => {
...
it('(EC-DOCS-12) keeps focused hidden-editor patches content-free', async () => {
    store.dispatch(
        hydrateProjection({
            revision: 1,
            documents: {
    ...
    render(
        <Provider store={store}>
            <EditorSessionContext.Provider value={{ documentId: 'document-1', content: '# Local working copy' }}>
                <EditorView />
            </EditorSessionContext.Provider>
        </Provider>,
    );
    const editor = await screen.findByLabelText<HTMLTextAreaElement>('Markdown source');
    expect(screen.getByLabelText('Editor pane')).toHaveAttribute('aria-hidden', 'true');
...
it('flushes session state before hiding the editor and does not hide after a flush error', async () => {
    ...
    const mockedAdapter = appModelAdapter as jest.Mocked<typeof appModelAdapter>;
    mockedAdapter.getState.mockResolvedValue(initialState);
    mockedAdapter.flushBuffer.mockImplementationOnce((): Promise<void> => bufferFlush.promise);
    mockedAdapter.flushDocView.mockImplementationOnce((): Promise<void> => viewFlush.promise);
...
it('acknowledges buffer and view before hiding the persistent session', async () => {
    const document = statusDocument({
        view: {
            arrangement: 'editor',
            editorVisible: true,
    ...
    mockedAdapter.flushBuffer.mockImplementationOnce((): Promise<void> => bufferFlush.promise);
    mockedAdapter.flushDocView.mockImplementationOnce((): Promise<void> => viewFlush.promise);
    mockSetDocView.mockImplementationOnce((): Promise<void> => commandAck.promise);
...
it('(EC-DOCS-12) preserves newer local view values and focused Monaco content and selection during patch and flush', async () => {
    const document = statusDocument({
        view: {
            arrangement: 'editor',
            editorVisible: true,
...
it('isolates pending view intent across document switch', async () => {
    ...
    expect(screen.getByLabelText('Markdown source')).toHaveValue('# Second working copy');
    ...
    expect(mockSetDocView).not.toHaveBeenCalledWith(second.documentId, expect.anything());
    expect(screen.getByLabelText('Markdown source')).toHaveValue('# Second working copy');
});

it('keeps Monaco mounted while preview-only is visible', async () => {
    const document = statusDocument({
        view: {
            arrangement: 'editor',
            editorVisible: true,
    ...
    expect((mountedEditor as unknown as { dispose: jest.Mock }).dispose).not.toHaveBeenCalled();
});

// view state; per-document dirty state and arrangement are proved by the // sibling at the end of this file)
it('restores the exact Monaco session and saved scroll state without bootstrap reseeding', async () => {
    const document = statusDocument({
        view: {
            arrangement: 'editor',
            editorVisible: true,
    ...
    const applyAcceptedView = (
        revision: number,
        view: Pick<DocumentMetadata['view'], 'arrangement' | 'editorVisible' | 'previewVisible'>,
    ): void => {
        act((): void => {
            mockStatePatchListener?.({
                revision,
    ...
    expect(mockedEditor.restoreViewState).toHaveBeenNthCalledWith(1, mockRuntime.viewState);
    expect(mockedEditor.restoreViewState).toHaveBeenNthCalledWith(2, mockRuntime.viewState);
...
it('round trips view mode through the backend patch', async () => {
...
// filed as . Caret, selection and editor view state are proved by
// 'restores the exact Monaco session and saved scroll state'.)
//
...
it('keeps each document its own dirty state and arrangement across a switch', async () => {
    const first = {
        ...statusDocument({
            dirty: true,
    ...
    const rendered = render(
        <Provider store={store}>
            <EditorSessionContext.Provider value={{ documentId: first.documentId, content: '# First' }}>
                <EditorView />
            </EditorSessionContext.Provider>
        </Provider>,
    );
    ...
    const switchTo = (document: typeof first, content: string, revision: number): void => {
        act((): void => {
            store.dispatch(
                hydrateProjection({
                    revision,
 ...
 */
function lastDocViewFor(documentId: string): { scroll?: { editor: number; preview: number } } | undefined {
    const updates = (appModelAdapter as jest.Mocked<typeof appModelAdapter>).updateLocalDocView as unknown as jest.Mock;
    const calls = updates.mock.calls.filter((call) => call[0] === documentId);
    return calls.at(-1)?.[1] as { scroll?: { editor: number; preview: number } } | undefined;
}

/*
 * — 's "Editing … MUST be unavailable", at Monaco itself.
 *
 ...
 */
it('makes the editor widget read-only for a non-writable document', async () => {
...
it('leaves the editor widget writable for a writable document', async () => {
    renderLivePreviewEditor('fine', createRenderedEditorAdapter(), statusDocument({ capability: 'writable' }));
...
/*
 * — 's "preview scroll" clause, the half could not cover.
 *
 ...
 */
it('restores a document its own preview scroll on activation', async () => {
    const document = statusDocument({
        documentId: 'document-1',
        view: {
            arrangement: 'split',
    ...
    const content = preview.querySelector('div[class*="previewContent"], div:not([class])');
...
it('does not re-apply the saved preview scroll when the content changes', async () => {
    ...
    const { rerender } = renderLivePreviewEditor('# body', createRenderedEditorAdapter(), document);
    ...
    rerender(
        <Provider store={store}>
            <EditorSessionContext.Provider value={{ documentId: 'document-1', content: '# body edited' }}>
...
/*
 * — 's scale clause: "Across **repeated switches among 40
 ...
 *
 * Preview scroll is deliberately excluded: restores it once per
...
// editor is mocked. Preview scroll is 's.)
it('restores each of forty documents its own view across repeated switches', async () => {
    ...
    const rendered = render(
        <Provider store={store}>
            <EditorSessionContext.Provider value={{ documentId: ids[0] as string, content: '# 0' }}>
                <EditorView />
            </EditorSessionContext.Provider>
        </Provider>,
    );
    const switchTo = (documentId: string): void => {
        ...
        rendered.rerender(
            <Provider store={store}>
                <EditorSessionContext.Provider value={{ documentId, content: `# ${documentId}` }}>
                    <EditorView />
                </EditorSessionContext.Provider>
            </Provider>,
        );
    ...
    const expectRestored = async (index: number): Promise<void> => {
        ...
        if (expected === undefined) throw new Error(`missing fixture ${documentId}`);
        switchTo(documentId);
        // Arrangement is the document's own, not the application fallback.
        await waitFor((): void => {
            expect(
                screen.getByRole('radio', {
                    name: arrangementLabel[expected.view.arrangement as keyof typeof arrangementLabel],
                }),
            ).toBeChecked();
        });
