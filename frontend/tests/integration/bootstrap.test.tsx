    ...
    return (
        <>
            ...
            <output aria-label="Bootstrap failure">
                {failure === null ? '' : `${failure.step}:${failure.category}:${String(failure.timedOut)}`}
            </output>
...
it.each<BootstrapStep>(['bridge', 'model', 'settings', 'window-ready'])(
    ...
    async (heldStep) => {
        ...
        const windowReady = heldStep === 'window-ready' ? jest.fn(() => new Promise<void>(() => undefined)) : undefined;
        if (windowReady !== undefined) {
            services.windowAdapter.windowReady = windowReady;
        }

        render(<Harness bootstrapModel={model} bootstrapSettings={settings} loadAdapters={loadAdapters} />);
        ...
        await waitFor(() => {
            expect(screen.getByRole('status', { name: 'Bootstrap status' })).toHaveTextContent('failed');
        });
        expect(screen.getByRole('status', { name: 'Bootstrap failure' })).toHaveTextContent(`${heldStep}:`);
    },
);
...
it('retries only the failed step and ignores a second retry while it is running', async () => {
    ...
    const loadAdapters = jest.fn(async (): Promise<BootstrapAdapters> => services);
    ...
    expect(screen.getByRole('status', { name: 'Bootstrap status' })).toHaveTextContent('ready');
...
it('uses the failure step carried by a failed model bootstrap result', async () => {
    ...
    expect(screen.getByRole('status', { name: 'Bootstrap failure' })).toHaveTextContent('settings:internal:false');
});
