...
/*
 * gap 1 — the roving tabindex at DocumentTabs.tsx:505 (`tabIndex={active ? 0 : -1}`)
...
/*
 * gap 2 — Home and End are handled on the tablist (DocumentTabs.tsx:465) and move
...
/*
 * gap 2 — ArrowLeft/ArrowRight on the tablist (DocumentTabs.tsx:474) activate the
...
/*
 * gap 2 — only next-tab was ever pressed anywhere in the suite. Both bindings are
...
/*
 * gap 3 — : Copy path copies the exact canonical absolute path and
...
/*
 * gap 3 — : a clipboard write failure reports one deduplicated
...
/*
 * gap 4 — : OS acceptance of a Reveal is success and MUST produce no
...
/*
 * gap 4 — : an OS command failure for Reveal reports one deduplicated
 * `system-command-failure` that additionally offers Copy path.
 *
 * corrected the fixture as well as the mapping: Go has sent the full pair
 * `['Retry', 'Copy path']` since (`internal/appmodel/copy_path.go`,
...
/*
 * gap 6 — the only thing a detached document changes on the tab surface is the
 * availability of Reveal. : a document already known missing shows Reveal as
...
/*
 * gap 7 — the tab half of the committed Save As outcome. The adopted path arrives
...
/*
 * + the classified-error table: `capacity-limit` is remediated
...
/*
 * gap (a). closes with a clause nothing implemented: "After a
...
/*
 * . names three affordances that close the *targeted* tab — the
...
/*
 * . `role="tab"` requires an owning `tablist`, and a plain `<div>` between
 ...
 * tabIndex and the Home/End/Arrow handler traverse. already covers that
 * behaviour on its own terms; this case re-anchors it to so the ownership
...
/*
 * . 's focus chain has four steps and the fourth had no code:
...
/*
 * . binds Move tab left/right to `Ctrl/Cmd+Shift+PageUp` and
...
/*
 * . A reorder Retry has two halves and the contract is only served if both
 * run: the move must be re-issued against a *fresh* revision, and 's
 * announcement must still be made. declined to add the intent precisely
 ...
 * tab and said nothing would satisfy the remediation row and break .
 *
...
/*
 * The genuinely new rule. : "Moving past an edge MUST succeed as a
...
/*
 * , — the pointer drag. The five clauses are split across two
 ...
 * here and not in a browser. The two clauses deferred and built — the
...
/*
 * directionally isolates user-supplied path text, so every tab's
 ...
 * is two commands. requires each to carry the tab-set revision it was
...
/*
 * : "issue no reorder for a same-position drop". The weak reading is
...
/*
 * . Go refuses a switch against a stale tab set with `conflict` and sends
 ...
 * 're-activates the named tab against a fresh revision' proves that half.
 */
...
/*
 * : "A tab switch MUST flush and await the outgoing document's newest
