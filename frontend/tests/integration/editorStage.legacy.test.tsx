...
jest.mock('../../src/logic/adapter', () => ({ appModelAdapter: mockAdapter }));
...
jest.mock('../../src/ui/components/CodeEditor', () => {
    ...
    >(function MockCodeEditor({ activationId, initialValue }, ref): React.JSX.Element {
...
import type { DocumentMetadata, ViewArrangement } from '../../src/logic/store/appModelTypes';
...
it('applies the responsive split layout contract', () => {
    render(
        <Provider store={store}>
...
it('keeps only the editor at the minimum window in Editor mode', () => {
...
it('collapses Split to the editor at the minimum window and removes the preview', () => {
    setViewportWidth(375);
    try {
        renderEditorView('split');
...
it('restores Split when the window widens again without writing an arrangement', () => {
    ...
    const rendered = render(
        <Provider store={store}>
            <EditorSessionEpochContext.Provider value={0}>
                <EditorSessionContext.Provider value={{ documentId: document.documentId, content: '# mine\n' }}>
                    <EditorView />
                </EditorSessionContext.Provider>
    ...
    rendered.rerender(
        <Provider store={store}>
            <EditorSessionEpochContext.Provider value={1}>
                <EditorSessionContext.Provider value={{ documentId: document.documentId, content: '# disk\n' }}>
                    <EditorView />
                </EditorSessionContext.Provider>
            </EditorSessionEpochContext.Provider>
        </Provider>,
...
it('prevents an empty document arrangement', async () => {
    renderEditorView('split');
    ...
    expect(requestedViews.every((view): boolean => view.editorVisible || view.previewVisible)).toBe(true);
});

it('matches the split-view structure', () => {
    renderEditorView('split');
    ...
    expect(within(arrangement).getByRole('radio', { name: 'Split' })).toBeChecked();
...
/*
 * . Three `` cases were removed here with the five CSS rules they
 ...
 * in it. That is only worth doing if the declaration does something, and  * measured that it does not: those selectors match only when the parity harness
