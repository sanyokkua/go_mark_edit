import { execFile as execFileCallback, execFileSync, spawn, type ChildProcessByStdio } from 'node:child_process';
...
function parsePidFromOutput(output: string): number | undefined {
    const match = output.match(/\b(?:app(?:lication)?\s+)?(?:child\s+)?(?:pid|process\s+id)\s*[:=]\s*(\d+)\b/iu);
...
function descendantProcessIDs(rootPID: number, processes: readonly ProcessSnapshot[]): number[] {
    const descendants: number[] = [];
    const parents = [rootPID];
    for (const parentPID of parents) {
        for (const process of processes) {
            if (process.parentPid !== parentPID || descendants.includes(process.pid)) {
                continue;
            }
...
function isWailsInfrastructure(command: string): boolean {
    return /(?:^|[\s/])(?:wails|npm|node|vite|go|bash|zsh|sh)(?:$|[\s])/iu.test(command);
}
...
async function findAppChildPid(wailsPid: number, output: string): Promise<number | undefined> {
...
async function terminateProcessTree(child: WailsProcess | null): Promise<void> {
    ...
    if (process.platform === 'win32') {
        ...
        while (processIDs.some((processID) => processIsAlive(processID)) && Date.now() < deadline) {
            await sleep(100);
        }
...
function goEnvironment(): NodeJS.ProcessEnv {
    try {
        const values = execFileSync(process.env.GO_BIN ?? 'go', ['env', 'GOPATH', 'GOCACHE', 'GOMODCACHE'], {
            encoding: 'utf8',
        })
            .trim()
            .split('\n');
        ...
        return {
            ...(goPath === undefined || goPath.length === 0 ? {} : { GOPATH: goPath }),
            ...(goCache === undefined || goCache.length === 0 ? {} : { GOCACHE: goCache }),
            ...(goModuleCache === undefined || goModuleCache.length === 0 ? {} : { GOMODCACHE: goModuleCache }),
        };
    } catch {
        return {};
    }
...
function childEnvironment(tempDirectory: string, goPaths: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
...
class PlaywrightE2EAppHarness implements E2EAppHarness {
    ...
    async launch(): Promise<void> {
        ...
        this.devOutput = '';
        const child = spawn(process.env.WAILS_BIN ?? 'wails', ['dev', '-devserver', 'localhost:34115', '-nocolour'], {
    ...
    private async waitForBrowserPage(child: WailsProcess): Promise<void> {
        ...
        while (Date.now() < deadline) {
            if (child.exitCode !== null || child.signalCode !== null) {
                throw new Error(`wails dev exited before the browser page was ready.\n${this.devOutput}`);
            }
            try {
