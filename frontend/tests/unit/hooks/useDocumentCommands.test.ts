...
it('exposes editor changes through the document-command seam', () => {
    const selection: EditorSelection = {
        start: { lineNumber: 1, column: 2 },
        end: { lineNumber: 1, column: 4 },
    };
    const replaceRange = jest.fn<boolean, [EditorRange, string, EditorSelection?]>(() => true);
    ...
    const commands = createDocumentCommands(session.documentId, session.token, (): DocumentCommandSession => session);
...
it('returns the current selection or null', () => {
    const selection: EditorSelection = {
        start: { lineNumber: 3, column: 2 },
        end: { lineNumber: 3, column: 8 },
    };
    const session = createSession('document-1', createHandle({ getSelection: (): EditorSelection => selection }));
...
it('returns unavailable when no session handle exists', () => {
    const commands = createDocumentCommands('document-1', Symbol('expected'), () => null);
    const range: EditorRange = {
        start: { lineNumber: 1, column: 1 },
        end: { lineNumber: 1, column: 1 },
    };
...
it('discriminates detached unavailable and stale mismatch', () => {
...
it('returns unavailable without touching an unavailable Monaco model', () => {
...
function moduleName(expression: ts.Expression | undefined): string | null {
    return expression !== undefined && ts.isStringLiteral(expression) ? expression.text : null;
}
...
function hasMonacoModuleLoad(source: ts.SourceFile): boolean {
    let found = false;
    const visit = (node: ts.Node): void => {
        if (ts.isImportDeclaration(node) && isMonacoModule(moduleName(node.moduleSpecifier))) {
            found = true;
        }
        if (
...
it('continues to enforce the direct-Monaco ownership boundary', () => {
    const commandSource = sourceFile(resolve(process.cwd(), 'src/logic/hooks/useDocumentCommands.ts'));

    expect(hasMonacoModuleLoad(commandSource)).toBe(false);
    expect(interfaceMembers(commandSource, 'DocumentCommandAPI')).toContain('getSelection');
});

it('keeps sibling consumers independent of Monaco', () => {
