...
it('flushActiveSession accepts latest content and view', async (): Promise<void> => {
    ...
    const queueBuffer = jest.fn(async (documentId: string, content: string): Promise<void> => {
        order.push(`queue-buffer:${documentId}:${content}`);
    });
    const queueDocView = jest.fn(async (documentId: string, view: TestView): Promise<void> => {
    ...
    const result = await barrier.flushActiveSession('document-1', activationToken, () => captureSnapshot());
...
it('one failed queue aborts lifecycle', async (): Promise<void> => {
    ...
    await expect(barrier.flushActiveSession('document-1', activationToken, () => captureSnapshot())).rejects.toBe(
        failure,
    );
...
it('immediate Save waits for pending keystroke', async (): Promise<void> => {
    ...
    let settled = false;
    const save = barrier
        .flushActiveSession('document-1', activationToken, () => captureSnapshot('keystroke not yet flushed'))
        .then((): void => {
            settled = true;
        });
