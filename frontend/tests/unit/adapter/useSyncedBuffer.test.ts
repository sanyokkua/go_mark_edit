    ...
    const adapter = createAppModelAdapter(
        {
            ...
            updateBuffer,
            setDocView: async (documentId: string, view: DocViewInput): Promise<VoidResult> => {
                void documentId;
                void view;
                return {};
            },
...
it('flushes the latest buffer with ordered acknowledgement', async () => {
    jest.useFakeTimers();
...
it('publishes the identical accepted buffer generation', async () => {
    jest.useFakeTimers();
...
it('retains the working copy after synchronization failure', async () => {
    jest.useFakeTimers();
    const wireError = {
        code: 'internal',
    ...
    expect(updateBuffer).toHaveBeenNthCalledWith(2, 'document-1', 'local working copy');
