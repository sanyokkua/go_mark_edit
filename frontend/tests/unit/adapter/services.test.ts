...
it('preserves classified New/Open outcomes through guarded lifecycle bindings', async () => {
...
it('guards Save and Save As with their exact three-argument bridge shapes', async () => {
    const bindings: DocumentWriteBindings = {
        save: jest.fn(async (documentId: string, contentRevision: number, decisionToken: string) => {
            void documentId;
            void contentRevision;
            void decisionToken;
            return { status: 'cancelled' as const };
        }),
        saveAs: jest.fn(async (documentId: string, contentRevision: number, decisionToken: string) => {
            void documentId;
            void contentRevision;
            void decisionToken;
            return { status: 'cancelled' as const };
        }),
        // . Two arguments, not three: the release is bound to the document and
        ...
        cancelNormalization: jest.fn(async (documentId: string, decisionToken: string) => {
            void documentId;
            void decisionToken;
            return {};
        }),
    ...
    await expect(adapter.cancelNormalization('doc-1', 'decision-1')).resolves.toEqual({});
    expect(bindings.save).toHaveBeenCalledWith('doc-1', 8, '');
    expect(bindings.saveAs).toHaveBeenCalledWith('doc-1', 8, 'decision-1');
    expect(bindings.cancelNormalization).toHaveBeenCalledWith('doc-1', 'decision-1');
});

it('guards every conflict decision with its exact bridge argument order', async () => {
    const version = {
        exists: true,
        mode: 0o644,
        modifiedUnixNano: '9',
    ...
    const bindings: DocumentConflictBindings = {
        authorizeKeepMine: jest.fn(async (documentId, contentRevision, path, detectedVersion) => {
            void documentId;
            void contentRevision;
            void path;
    ...
    await expect(adapter.authorizeKeepMine('doc-1', 4, '/repo/doc-1.md', version)).resolves.toEqual({
        status: 'authorized',
    });
    ...
    expect(bindings.authorizeKeepMine).toHaveBeenCalledWith('doc-1', 4, '/repo/doc-1.md', version);
...
it('ClosePlan complete-plan bridge preserves choices and exact arity', async () => {
    const prepareClose: ClosePlanBindings['prepareClose'] = jest.fn(
        async (kind: ClosePlanKind, ids: string[], revision: number): Promise<ClosePlanResult> => {
    ...
    const executeClosePlan: ClosePlanBindings['executeClosePlan'] = jest.fn(async (planId: string) => {
    ...
    await expect(adapter.prepareClose('right', ['doc-1', 'doc-2'], 12)).resolves.toMatchObject({
        data: { id: 'plan-1', kind: 'right' },
    });
    await expect(
 ...
 * killed.
 */
it('tolerates a plan whose empty target list arrives as null', async () => {
    const prepareClose: ClosePlanBindings['prepareClose'] = jest.fn(
        async (kind: ClosePlanKind, ids: string[], revision: number): Promise<ClosePlanResult> => {
            void kind;
            void ids;
