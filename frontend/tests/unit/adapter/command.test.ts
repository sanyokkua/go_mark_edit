import { createCommandInvoker, type CommandNoticeOwner } from '../../../src/logic/adapter/command';
...
it('attaches a fresh bridge request id to every command call', () => {
    ...
    const invoker = createCommandInvoker({
        ...
        uuid: jest.fn<string, []>().mockReturnValueOnce('request-1').mockReturnValueOnce('request-2'),
    });
...
it('withdraws two bounded notices independently when their results arrive', async () => {
    ...
    const invoker = createCommandInvoker({
        isReady: (): boolean => true,
        noticeOwner,
        uuid: jest.fn<string, []>().mockReturnValueOnce('first-request').mockReturnValueOnce('second-request'),
    });
    const command = invoker.bind(
        'AppModelHandler.GetState',
...
it('retries a stuck command with the same request id and arguments', async () => {
    ...
    const command = invoker.bind(
        'AppModelHandler.OpenDocument',
        function openDocument(request: { id: string }, expectedRevision: number): Promise<string> {
            recorder.calls.push({
