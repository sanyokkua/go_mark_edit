...
it('guards and unwraps every settings call', async () => {
    ...
    await expect(
        Reflect.apply(
            guardArity('SettingsHandler.UpdateContentPrivacy', bindings.updateContentPrivacy),
            undefined,
            [],
        ) as Promise<void>,
    ).rejects.toThrow('SettingsHandler.UpdateContentPrivacy expects 1 argument(s), received 0.');
    await expect(
        Reflect.apply(
...
it('notifies exactly once before throwing the same wire error', () => {
    ...
    expect(store.getState().notifications.items).toEqual([expect.objectContaining({ error: wireError })]);
});
