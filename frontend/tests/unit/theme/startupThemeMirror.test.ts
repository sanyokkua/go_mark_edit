import { readStartupThemeMirror, writeStartupThemeMirror } from '../../../src/logic/theme/startupThemeMirror';
...
it('reads only valid theme and appearance choices with Material/Auto fallback', (): void => {
    ...
    localStorage.setItem('gme.theme', JSON.stringify({ version: 1, theme: 'glass', mode: 'dark' }));
    expect(readStartupThemeMirror(localStorage)).toEqual({
        theme: 'glass',
        mode: 'dark',
    });
    ...
    localStorage.setItem('gme.theme', JSON.stringify({ version: 0, theme: 'glass', mode: 'dark' }));
    expect(readStartupThemeMirror(localStorage)).toEqual({
        theme: 'material',
        mode: 'auto',
    });
