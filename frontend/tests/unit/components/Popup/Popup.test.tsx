...
function PopupHarness(): React.JSX.Element {
    ...
    return (
        <div className="application-frame">
            <PopupTrigger ref={setTrigger} expanded={open} onClick={(): void => setOpen((value) => !value)}>
                Open
            </PopupTrigger>
            <Popup anchor={{ trigger }} open={open} role="menu" size="menu" onOpenChange={setOpen}>
...
it('attaches outside-pointer dismissal only while open and preserves the portal boundary', async () => {
    ...
    const result = render(
        <div className="application-frame">
            ...
            <Popup anchor={{ point: { x: 40, y: 40 } }} open role="menu" size="wide" onOpenChange={onOpenChange}>
    ...
    await waitFor(() => expect(screen.getByRole('menuitem', { name: 'Only item' })).toHaveFocus());
...
it('focuses the first visible MenuItem when a responsive group is hidden', async () => {
    render(
        <Popup anchor={{ point: { x: 40, y: 40 } }} open role="menu" size="menu" onOpenChange={jest.fn()}>
