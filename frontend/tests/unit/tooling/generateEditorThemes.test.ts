...
it('rejects a palette missing a required Monaco token', () => {
    expect(() => generate(palette.replace('--hl-punct: #5c5c69;', ''))).toThrow(/--hl-punct/);
});
...
it('rejects duplicate, unresolved, and unsupported palette sources', () => {
    expect(() =>
        generate(palette.replace('--hl-punct: #5c5c69;', '--hl-punct: #5c5c69; --hl-punct: #000000;')),
    ).toThrow(/Duplicate token --hl-punct/);
    expect(() => generate(palette.replace('--hl-punct: #5c5c69;', '--hl-punct: var(--other);'))).toThrow(
        /Unresolved token --hl-punct/,
    );
    expect(() => generate(`${palette}\n:root[data-theme='retro'][data-mode='light'] { --hl-punct: #000000; }`)).toThrow(
