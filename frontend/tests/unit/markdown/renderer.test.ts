...
const remoteHTMLAssetSource = /<(?:script|link)\b[^>]*\b(?:src|href)\s*=\s*['"]\s*(?:https?:)?\/\//i;
...
it('renders accessible repeated GFM footnotes and backlinks', () => {
    ...
    for (const reference of references) {
        ...
        expect(container.querySelector(reference.getAttribute('href') ?? '')).toBe(definition);
    }
    ...
    const secondRenderReferenceIds = screen.getAllByRole('link', { name: '1' }).map((reference) => reference.id);
...
it('attempts zero runtime requests and keeps higher tiers literal (EC-RENDER-6)', () => {
    ...
    const scriptSourceDescriptor = Object.getOwnPropertyDescriptor(HTMLScriptElement.prototype, 'src');
    ...
    recordResourceAssignment(HTMLImageElement.prototype, 'src', imageSourceDescriptor);
    recordResourceAssignment(HTMLScriptElement.prototype, 'src', scriptSourceDescriptor);
    recordResourceAssignment(HTMLLinkElement.prototype, 'href', linkHrefDescriptor);
...
it('processes a document of exactly the 2 MiB preview limit', () => {
    ...
    const line = 'lorem ipsum dolor sit amet consectetur adipiscing elit '.repeat(20).slice(0, 1100);
    let source = '';
