...
it('keeps web, absolute, and outside image sources as placeholders', () => {
    expect(classifyImageSource('https://example.test/image.png', documentPath)).toMatchObject({
        kind: 'placeholder',
        reason: 'scheme',
    });
    expect(classifyImageSource('/tmp/notes/image.png', documentPath)).toMatchObject({
        kind: 'placeholder',
