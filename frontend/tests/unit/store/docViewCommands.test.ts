...
const mockFlushBuffer = jest.fn<Promise<void>, [string]>(async (): Promise<void> => undefined);
const mockFlushDocView = jest.fn<Promise<void>, [string]>(async (): Promise<void> => undefined);
...
it('keeps the confirmed arrangement and panes unchanged until an accepted backend patch arrives', async (): Promise<void> => {
    ...
    expect(store.getState().documents.byId[document.documentId].view).toEqual(document.view);
