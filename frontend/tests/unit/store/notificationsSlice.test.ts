...
function error(code: WireError['code'], title: string, message: string): WireError {
...
it('deduplicates by classified code plus subject even when safe copy changes', () => {
    ...
    const repeated = notifyError(error('io', 'Could not save', 'Check that the destination is writable.'));
...
it('never evicts visible errors and promotes queued errors in arrival order', () => {
    ...
    expect(full.items.map((item) => item.error?.title)).toEqual(['First', 'Second', 'Third']);
    expect(full.queuedErrors.map((item) => item.error?.title)).toEqual(['Fourth', 'Fifth']);

    const promoted = reducer(full, dismissNotification(full.items[0].id));
    expect(promoted.items.map((item) => item.error?.title)).toEqual(['Second', 'Third', 'Fourth']);
    expect(promoted.queuedErrors.map((item) => item.error?.title)).toEqual(['Fifth']);
});
...
it('displaces only the oldest non-error and drops non-errors behind three errors', () => {
    ...
    expect(state.items.map((item) => item.title)).toEqual(['File operation failed', 'Warning', 'Complete']);

    for (const title of ['Second error', 'Third error']) {
        state = reducer(state, notifyError(error('internal', title, 'Safe.'), title));
    }
...
it('autosave success emits no toast while refreshing continuing conditions', () => {
    ...
    state = reducer(state, notifyCondition({ ...condition, message: 'Still working locally.' }));
...
/*
 * . `refreshDuplicate` assigned the incoming remediation wholesale, so a
...
it('keeps the controls a notification earned when a later report brings none', () => {
    ...
    expect(state.items[0].remediations.map((offer) => offer.action)).toEqual(['retry', 'copy-path']);
});

// repeat which brings a control the notification does not yet have adds it once.
it('adds a newly offered control on a repeat without duplicating the existing ones', () => {
