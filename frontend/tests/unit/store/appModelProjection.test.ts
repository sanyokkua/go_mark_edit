...
function createAdapter(getState: () => Promise<AppModelState>): AppModelAdapter & {
...
it('dev bridge dirty state follows disk baseline', async (): Promise<void> => {
    ...
    await expect(bootstrapAppModelProjection(createAdapter(async () => state))).resolves.toMatchObject({
        status: 'ready',
    });

    expect(store.getState().documents.byId[documentMetadata.documentId]).toMatchObject({
        dirty: true,
        status: 'unsaved-changes',
    });
});
...
it('projects optional active state', async () => {
    ...
    await expect(bootstrapAppModelProjection(createAdapter(async () => state))).resolves.toEqual({
        status: 'ready',
        activeBuffer: null,
        applicationVersion: 'test-build',
    });
...
it('applies the layout section of a patch carrying a null tab order', async () => {
    ...
    expect(store.getState().documents.orderedIds).toEqual([documentMetadata.documentId]);
});
...
it('supports a fresh retry after each repeated failed attempt', async () => {
    ...
    const success = createAdapter(async (): Promise<AppModelState> => appState(7));
