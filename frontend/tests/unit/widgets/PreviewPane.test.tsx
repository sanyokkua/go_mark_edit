...
import PreviewPane, { PREVIEW_BYTE_LIMIT, type PreviewSnapshot } from '../../../src/ui/widgets/PreviewPane';

function snapshot(revision: number, content: string, byteLength: number): PreviewSnapshot {
...
it('presents source-backed paused preview chrome above the pane content', () => {
    ...
    expect(paused.textContent).toContain('Live preview is paused — this document is over 2\u00a0MB.');
    expect(within(paused).getByRole('button', { name: 'Refresh preview' })).toBeEnabled();
});
...
it('renders the accepted revision once and coalesces duplicate refresh requests', async () => {
    ...
    pending.resolve(snapshot(17, '# accepted revision 17', PREVIEW_BYTE_LIMIT + 1));

    await waitFor(() => {
        expect(screen.getByText('accepted revision 17')).toBeInTheDocument();
    });
    expect(screen.getByRole('region', { name: 'Preview pane' })).toHaveAttribute('data-preview-revision', '17');
});
...
it('keeps a failed refresh paused, classifies io-failure, and offers Retry', async () => {
    ...
    expect(screen.getByRole('alert')).toHaveAttribute('data-error-code', 'io-failure');
...
/*
 * . Two `…` cases were removed here: one asserting the preview showed
 ...
 * the invention. The difference now lives where puts it — the
