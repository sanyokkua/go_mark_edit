...
it('clamps context-menu placement and uses the selection captured at opening', async () => {
    ...
    await waitFor(() =>
        expect(commands.replaceRange).toHaveBeenCalledWith(expect.anything(), '**word**', expect.anything()),
    );
    bounds.mockRestore();
});
...
it('renders the context menu in the body-owned viewport popup layer', () => {
    render(
        <div style={{ filter: 'blur(4px)', overflow: 'hidden', transform: 'scale(1)' }}>
            <EditorContextMenu>
                <textarea aria-label="Markdown source" />
            </EditorContextMenu>
        </div>,
...
it('keeps the originating session callback available without tab or assistant state', () => {
    ...
    expect(screen.queryByRole('region', { name: 'Assistant' })).not.toBeInTheDocument();
});
...
it('renders platform-resolved accelerator metadata for context actions', () => {
    ...
    expect(screen.getByRole('menuitem', { name: 'Format document' })).toHaveAttribute(
        'aria-keyshortcuts',
...
it('passes acknowledged marker preferences into context formatting', () => {
    ...
    expect(replaceRange).toHaveBeenCalledWith(expect.anything(), '_word_', expect.anything());
});
