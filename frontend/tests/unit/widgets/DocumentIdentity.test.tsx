...
import type { DocumentMetadata, SaveStatus } from '../../../src/logic/store/appModelTypes';
import DocumentIdentity from '../../../src/ui/widgets/DocumentIdentity';
...
it('DocumentIdentity renders all five status values', () => {
    const statuses: SaveStatus[] = ['not-saved', 'unsaved-changes', 'saved', 'autosaved', 'read-only'];
    const labels = ['Not saved', 'Unsaved changes', 'Saved', 'Autosaved', 'Read-only'];
    for (const [index, status] of statuses.entries()) {
        const { unmount } = render(<DocumentIdentity document={documentFor(status)} />);
...
it('identity heading keeps at most one parent segment', () => {
    render(<DocumentIdentity path="/one/two/three/name.md" parentName="two/three" status="not-saved" />);

    const heading = screen.getByRole('heading');
    expect(heading).toHaveTextContent('twothree / name.md');
    expect((heading.textContent?.match(/\//gu) ?? []).length).toBeLessThanOrEqual(1);
});
...
it('keeps the title-bar status to the bare state, without the reason', () => {
    /*
