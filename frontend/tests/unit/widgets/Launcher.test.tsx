...
it('Launcher first-run and six recent files', () => {
    ...
    const { rerender } = render(
        <Launcher onNewDocument={onNewDocument} onOpenDocument={onOpenDocument} onOpenRecentFile={onOpenRecentFile} />,
    );
    ...
    const recentFiles = Array.from({ length: 7 }, (_, index) => `/tmp/file-${index}.md`);
    rerender(<Launcher recentFiles={recentFiles} onOpenRecentFile={onOpenRecentFile} />);
...
it('renders translated launcher strings without invented path labels', () => {
    ...
    expect(screen.getByRole('button', { name: 'Open Folder' })).toBeDisabled();
    expect(screen.getByRole('heading', { level: 2 })).toHaveTextContent('Recent files');
