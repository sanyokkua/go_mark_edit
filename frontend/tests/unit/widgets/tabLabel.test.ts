...
it('control and bidirectional characters render as visible escapes', () => {
    expect(escapeUnsafeText(`bad\u0000\u007f\u202e.md`)).toBe('bad\\u0000\\u007F\\u202E.md');
    expect(
        tabLabelFor(documentFor('hostile', '/tmp/bad\u202E.md'), [documentFor('hostile', '/tmp/bad\u202E.md')]).label,
    ).toContain('\\u202E');
});
...
it('ellipsis retains a distinguishing suffix and the complete label stays accessible', () => {
    const label = tabLabelFor(documentFor('one', '/repo/first/very-long-name.md'), [
