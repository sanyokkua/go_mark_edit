...
import { createShellActionCatalogue, dispatchShellAction } from '../../../src/logic/actions/shellActions';
...
it('exposes one unique localized catalogue for every working shell action', async () => {
    ...
    expect(actions.map((action) => action.id)).toEqual(['settings', 'view', 'about', 'fullscreen']);
    expect(new Set(actions.map((action) => action.id)).size).toBe(actions.length);
    expect(new Set(actions.flatMap((action) => (action.shortcut === undefined ? [] : [action.shortcut]))).size).toBe(2);
    expect(actions.map((action) => action.scope)).toEqual(['application', 'window', 'application', 'window']);
...
it('includes the registry-owned Settings binding in shell dispatch', () => {
    const context = actionContext();
    const settings = createShellActionCatalogue(context).find((action) => action.id === 'settings');
