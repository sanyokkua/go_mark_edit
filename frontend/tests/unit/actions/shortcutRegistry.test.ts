...
it('shortcut registry has no duplicate binding', () => {
    const bindings = [...Object.values(shortcutRegistry), ...Object.values(shortcutAliases).flat()];
...
it('no jump-to-tab-by-number binding exists', () => {
    expect(Object.keys(shortcutRegistry).some((id) => /^tab-[0-9]+$/.test(id))).toBe(false);
});
