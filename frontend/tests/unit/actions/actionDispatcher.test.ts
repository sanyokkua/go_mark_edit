...
it('rejects unfocused application and window actions before invocation', async () => {
    ...
    await expect(dispatchAction('settings', { applicationFocused: false, invoke })).resolves.toMatchObject({
        actionId: 'settings',
        reason: 'unsupported',
        status: 'unavailable',
    });
    await expect(dispatchAction('toggle-sidebar', { windowFocused: false, invoke })).resolves.toMatchObject({
...
it('requires explicit focused context for application and window dispatch', async () => {
    ...
    await expect(dispatchAction('toggle-sidebar', { invoke })).resolves.toMatchObject({
        actionId: 'toggle-sidebar',
        reason: 'unsupported',
        status: 'unavailable',
    });
...
it('dispatches New and Open as focused application commands', async () => {
    ...
    await expect(dispatchAction('new-file', { applicationFocused: true, invoke })).resolves.toMatchObject({
        status: 'mutated',
        actionId: 'new-file',
    });
    await expect(dispatchAction('open-file', { applicationFocused: true, invoke })).resolves.toMatchObject({
        status: 'mutated',
        actionId: 'open-file',
    });
...
it('dispatches Exit as a focused application command', async () => {
    ...
    await expect(dispatchAction('exit', { applicationFocused: true, invoke })).resolves.toMatchObject({
        status: 'mutated',
        actionId: 'exit',
    });
...
it('dispatches Refresh preview as an available window action without a shortcut', async () => {
    ...
    await expect(dispatchAction('refresh-preview', { windowFocused: true, invoke })).resolves.toMatchObject({
        status: 'mutated',
...
it('refuses to dispatch a write command for an unsafe-read-only document', async () => {
    ...
    for (const id of ['save', 'save-as'] as const) {
        const invoke = jest.fn();
        await expect(dispatchAction(id, { projectedState, documentId: 'unsafe', invoke })).resolves.toMatchObject({
            status: 'unavailable',
            actionId: id,
            reason: 'no-document',
        });
