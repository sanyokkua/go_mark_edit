...
it('interpolates status labels and formats word counts by active locale', () => {
    ...
    expect(translator.t('status.cursor', { column: 8, line: 3 })).toBe('Zeile 3, Spalte 8');
    expect(translator.t('status.words', { count: 1200 })).toBe('1200 Woerter');
    expect(translator.formatNumber(1200)).toBe(new Intl.NumberFormat('de').format(1200));
...
it('EC-I18N-1 falls back to English and then the key without a blank label', () => {
    ...
    expect(translator.t('status.cursor', { column: 2, line: 4 })).toBe('Ln 4, Col 2');
