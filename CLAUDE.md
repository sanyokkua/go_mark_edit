...
- `.claude/agents/*.md` — the agent pipeline (investigator → architect → coder → tester → reviewer → docs).
...
Two custom commands (`.claude/commands/`) bootstrap the whole process. Both run **read-only in planning
...
Before assuming a convention doesn't exist: check `.claude/skills/`, the relevant `.claude/rules/*.md`
...
`06_Process_and_Traceability/07_PHASE_FORMAT.md`, and the `.claude/` rules/skills/agents are the
