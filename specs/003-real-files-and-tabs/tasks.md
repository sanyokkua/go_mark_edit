...
- [ ] T106 Finish the walkthrough checks the 2026-08-14 automated run left, per SC-FT-002 and SC-FT-011 (missing). Fixtures are already built, byte-exact, in `~/Documents/gomarkedit-walkthrough/`: the 10 MiB pair (`10,485,760` / `10,485,761`), the 50 MiB pair (`52,428,800` / `52,428,801`), and `fixture-b.md` at 62,826 bytes. Remaining: open each boundary file and record the classification either side of both limits; open 40 documents and record the 41st being refused; time Fixture B's explicit save under whichever method T105 settles on. `evidence/ft-ev-09/sc-ft-002/` already holds the before and after directory listings that prove FR-FT-009 leaves no temp file.
  ...
  - **Still open: the 40-document limit at the interface level**, and the reason is the instrument. The application surfaces no document count anywhere, the tab strip scrolls without a total, `New File` gives no distinguishable success or failure, the workspace panel is not a document list, and the layout is not persisted to `settings.db` where it could be counted; a clean-restart count was interrupted by another application taking focus mid-batch. The cap itself is well covered at the model level and green — `TestOpenRefusesFortyFirstWithoutMutation` asserts the 41st is refused with `ClassifiedCapacityLimit`, the document count and order unchanged, **and no patch emitted** — so what is owed is interface confirmation only. Use a Playwright case on the deterministic parity route, which can read the count from the DOM, rather than screen automation.
  - **Two observations needing confirmation, not filed as defects.** FR-FT-005 requires the 50 MiB refusal to carry "a message naming the 50 MiB limit" and requires read-only to open "with a visible reason". Neither was observed — the refusal was silent across both a 4-second and a 25-second observation window with nothing in the application log, and the only banner on a read-only document explains the preview pause. A transient toast or a reason inside `Document details`, which would not expand when clicked, cannot be excluded, and failing to see something is weaker evidence than the T104 disk measurement was.
