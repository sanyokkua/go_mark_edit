...
**Input**: User description: "Create a new specification/feature where we refactor the current codebase. No new functionality is expected. Fix the issues described in the 2026-09-07 project health audit: fix all known bugs; implement common components and reuse them; clean up the tests, removing bureaucracy tests and keeping only real functionality tests; fix the e2e tests so they run against the real backend instead of a stub; remove god objects and fix folder and component structure; fix code documentation and remove requirement and task IDs from code and tests; rewrite the project-level agent docs, skills, rules and commands (not the Spec Kit files); rewrite the scripts; clean the repository of generated temporary files and screenshots; remove the old `feature/v1-implementation*` branches, using `app_version_1_codebase` as the release branch and `feature/<NNN>-<short-description>[-<task>]` for feature and task branches."
...
On 2026-09-07 the owner audited the project (project health audit, revision 2.2). Its findings, which each user story below details:
...
- Q: What does `scripts/format` leave alone? → A: Only tool-owned paths: `.specify/**`, generated bindings, build output, lockfiles, the `justfile` (whose only formatter is `just` itself) and disposable `specs/*/evidence/**` artifacts; ordinary `specs/**` and every other tracked text file are formatted. (FR-063)
...
- Q: Are the legacy phase/story workflow files deleted or rewritten? → A: Deleted, with every reference removed; the Spec Kit skills are the only workflow; of the Spec Kit files only the constitution is versioned. (FR-077)
- Q: Which files must be free of task and requirement labels? → A: Go and TypeScript production source, every test file, `README.md`, `AGENTS.md`, `CLAUDE.md`, `docs/architecture.md` and Go package docs. `specs/**` and `.specify/**` keep them. (FR-080)
...
Owner decisions that restore synchronized scrolling between the editor and the preview. Features 001–002 intended it (feature 002 deferred it), and it was lost when the earlier specifications were generated.

- Q: Is synchronized scrolling between the editor and the preview part of this feature? → A: Yes; it restores a capability the earlier features intended and never delivered, and it is the second capability this feature adds, beside the local images of FR-049. (FR-085, FR-086)
- Q: How closely do the panes follow each other? → A: By rendered block: every block in the preview maps to the Markdown source line it starts on, positions between blocks are interpolated, and both panes start and end together; the heading-only design of the archived specification and proportional scrolling are not used. (FR-085)
- Q: Can synchronized scrolling be turned off, and is it on by default? → A: A "Synchronized scrolling" checkbox in the View menu, directly after Word wrap, stored as an editor setting and on by default. (FR-086)
...
- _"E2E" never reaches Go._ `playwright.config.ts` starts Vite with the mock bridge (`frontend/src/dev/bridge-mock`, 2,351 lines re-implementing all 24 bridge bindings) and a second server for the static HTML mockup. The legacy known-issues list recorded the divergence on 2026-07-25; it was never resolved. The only real-backend driver, `cmd/native-evidence` (about 2,169 build-tagged lines, a second composition root), is run by nothing.
- _One fifth of test code compares pixels._ `e2e/parity/**`, `targeted-parity`, `real-files-parity` and `interactive-states` total 9,901 lines; the first run of the 546-case harness scored 0 of 1,620. All eleven Playwright suites fail to start without the legacy delivery tree's mockup.
...
- _Layering._ `ui/primitives/ViewMenu.tsx` imports the action registry, the dispatcher and store types; `Banner`, `StatusBar` and `ViewModeToggle` import store types. The rule "components take props" exists in the legacy architecture rules but is not enforced.
...
- _Four answers to "what is normative"._ `README.md` points to the delivery tree; the delivery tree's README says nothing outside it is normative; `AGENTS.md` points to `specs/<feature>/`; the constitution says the delivery specification and architecture "until migrated"; the legacy workflow document still routes through `/plan-story`.
- _Dead references._ `just story-check` (seven sites; one test asserts it must not exist), `just spec-check`, `check_story.py`, `check_proves.py`, `sync-agent-files.py --apply`, `.claude/commands`, `jest.config.js`, "the 10 speckit skills" (there are 17), two stale known-issues entries.
...
- _No ownership guidance._ Nothing tells an agent to find the existing owner of a behaviour and its consumers before adding a copy; the legacy structure document prescribes folders and "inline until there is a second caller", which produced the copies.
- _Comments and docs._ 489 task and requirement IDs in production comments; `ShellMenuRow` takes about 40 props with no contract; `internal/appmodel` has no package documentation for lifecycle, lock order or event contract; the legacy structure document describes July's tree; `README.md` says `just check` is "everything CI runs" while CI also runs Playwright.
- _The constitution._ Principle I names the delivery tree as authority; Principle II requires stable requirement anchors and tests that name the rule they prove; Principle VII requires the baseline-gate ritual. All three conflict with the owner's principles 4, 5 and 8.
...
1. **Given** `README.md`, `AGENTS.md`, `CLAUDE.md` and the constitution, **When** they are read, **Then** all name `specs/<feature>/` plus `docs/architecture.md` as the single authority.
2. **Given** the legacy delivery tree, **When** the feature is done, **Then** it does not remain in the repository, and the decisions and ADRs still relevant are carried into the architecture map.
...
- `specs/` is 42 MB, almost all under `specs/*/evidence` (21 MB for 002, 9.9 MB for 003), including two 7.5 MB screenshots; the audit's evidence folder is 14 MB including a 4.3 MB PDF and a 3.6 MB JSON report; 387 tracked paths contain `evidence/`.
...
1. **Given** the repository after this feature, **When** tracked files are listed, **Then** no `specs/*/evidence`, `specs/*/surface/*.png`, `test-results/`, `frontend/evidence`, `cmd/native-evidence` or `frontend/public/*.test.*` path remains, while the product icons under `build/` and `frontend/src/ui/icons/file-tab-icons.svg` remain.
...
### User Story 8 - The editor and the preview scroll together (Priority: P2)

A person writing in Split view scrolls the Markdown source and the preview shows the same part of the document; scrolling the preview brings the editor to the same place. Features 001–002 intended this (feature 002 deferred it) and it was lost when the earlier specifications were generated; the owner restored it in this feature on 2026-09-15.

**Why this priority**: it restores an intended capability without changing any accepted behaviour, so it follows the defect fixes of Stories 1 and 2 and the shared components of Story 3.

**Independent Test**: open a long document in Split with synchronized scrolling on, scroll each pane and compare the block at the top of both panes; turn the View-menu row off and relaunch. The real-backend E2E case 9 and walkthrough step 16 cover it.

**Acceptance Scenarios**:

1. **Given** a long document in Split with synchronized scrolling on, **When** the user scrolls the editor by wheel, scrollbar, keyboard or caret movement, **Then** the preview shows at its top the rendered block whose source is at the top of the editor, within four source lines.
2. **Given** the same document, **When** the user scrolls the preview or activates an in-document anchor, **Then** the editor shows at its top the source line of the block at the top of the preview, within four source lines.
3. **Given** either pane scrolled to its end, **When** the scroll settles, **Then** the other pane is at its end too, and returning to the top brings both panes to the top.
4. **Given** both panes have stopped moving, **When** 700 ms pass without input, **Then** neither pane moves, and no content, selection or focus has changed.
5. **Given** the Editor or Preview arrangement, the minimum window width, a preview paused above 2 MiB or a document too short to scroll, **When** the user scrolls the visible pane, **Then** no other pane moves.
6. **Given** the View menu, **When** the user turns "Synchronized scrolling" off, **Then** the panes scroll independently at once and the row is still off after a relaunch; **and When** the user turns it on again, **Then** the preview aligns to the editor.
...
- A pane has nothing to scroll while synchronized scrolling is on (a short document, or a preview paused above 2 MiB): neither pane moves the other.
- The preview re-renders while synchronized scrolling is on (the text changes or an image finishes loading): the pane the user is scrolling keeps its place and the other pane realigns to it; a re-render is never taken for the user scrolling a pane.
...
- **FR-072**: The repository shall have one normative tree, `specs/<feature>/` plus the architecture map at `docs/architecture.md`, and `README.md`, `AGENTS.md`, `CLAUDE.md` and the constitution shall all name both. _Source: PR-1, DOC-4, DOC-5, D5, RF-F1._
- **FR-073**: The legacy delivery tree shall not remain in the repository, and the decisions and ADRs still relevant shall be carried into the architecture map. _Source: D5, H-8, RF-F1._
...
- **FR-077**: The legacy phase/story workflow (`.agents/commands/*`, the five legacy skills, their `.claude/skills` symlinks, the workflow and definition-of-done documents) shall be deleted with every reference to it, the Spec Kit skills shall be the only workflow, and of the Spec Kit files only the constitution shall be versioned. _Source: PR-5, AG-5, RF-F3._
...
- **FR-082**: The repository shall no longer track `specs/*/evidence`, `specs/*/surface/*.png`, `test-results/`, tracked screenshots and probe logs (the product icons under `build/` and `frontend/src/ui/icons/file-tab-icons.svg` stay), `frontend/evidence`, `cmd/native-evidence` and `frontend/public/*.test.*`. _Source: §5.8, TL-10, owner decision._
...
#### H. Synchronized scrolling (restored capability)

- **FR-085**: While synchronized scrolling is on and the editor and the preview are both shown, when the user scrolls either pane by any means (wheel or trackpad, scrollbar, keyboard, caret movement, find, a preview anchor), the application shall scroll the other pane so that the rendered block at the top of the preview is the one whose Markdown source is at the top of the editor, interpolating between blocks, with both panes reaching their top and their bottom together; the other pane shall follow on the next rendered frame with no animation, shall never oscillate, and shall change no content, selection or focus; synchronization shall not run in the Editor or Preview arrangement, at the minimum window width where one pane is shown, while the preview is paused, or when either pane has nothing to scroll; this restores the scroll synchronization that feature 002 deferred and supersedes its heading-only design (owner decision 2026-09-15). _Source: owner decision 2026-09-15._
- **FR-086**: The View menu shall offer a "Synchronized scrolling" checkbox directly after Word wrap, amending the View menu row list of feature 002 (FR-ED-004); the choice shall apply immediately, be stored as the editor setting `editor.scrollSync` (default on, a missing or invalid stored value read as on) and persist across restarts (owner decision 2026-09-15). _Source: owner decision 2026-09-15._
...
- **SC-013**: The application's capability inventory (window, themes, settings, editor, preview, files, tabs, autosave, conflicts, close, recents) is unchanged and no new capability is present except the local image rendering of FR-049 and the synchronized scrolling of FR-085 and FR-086; every deleted or rewritten suite has its coverage named in the plan before deletion.
...
- **SC-016**: With synchronized scrolling on in Split, after either pane is scrolled the other pane shows the same rendered block at its top within four source lines, both panes reach their top and bottom together, and neither pane moves again within 700 ms of the input stopping (real-backend E2E case 9, zero failures); with the setting off the other pane does not move, and the choice survives a relaunch.

## Assumptions

- **Approved behaviour is the baseline.** Features 001–003 as accepted define what the application does. This feature restores, fixes, consolidates and removes; it adds nothing except the local image rendering of FR-049, accepted in the final review, and the synchronized scrolling of FR-085 and FR-086, restored by owner decision on 2026-09-15. Where a shared component would change accepted behaviour, the change is recorded as a decision first.
- **Owner decisions on the audit's open items (2026-09-08)**: D1 toolbar overflow (FR-037); D2 square icon buttons (FR-038); D3 black-box Go tests under `tests/go` (FR-022, FR-023); D4 and D6 parity harness and mock bridge deleted once the real-backend E2E runs (FR-032); D5 `specs/` plus the architecture map as the only authority, the legacy delivery tree retired (FR-072, FR-073); D7 remote content and the offline wording (FR-049); D8 and D10 disabled controls and accepted amendments kept (FR-048); D9 native window frame (FR-050); D11 link policy (FR-014); D12 lost-screen handling with no session restore (FR-018).
...
- **Spec Kit installs are not versioned.** The `speckit-*` skills, `.specify/`, its templates and its extensions are owned by the Spec Kit CLI; of these, only `.specify/memory/constitution.md` is versioned.
...
- **The audit is read-only input.** The 2026-09-07 project health audit and its evidence folder are not versioned; this feature cites the audit's findings by their identifiers.
