...
- **End-to-end (E2E) test**: a test that starts the real application (Go backend plus frontend) and drives it like a user; the `wails dev` tier drives the screen in Chromium at the local address `wails dev` serves, the packaged-binary tier drives the native window from outside.
...
- Q: Must every one of the six real-backend E2E cases also run against the packaged binary from the outside, or only the cases where an outcome is visible from outside the application? → A: Every case runs on `wails dev`; a packaged-binary version exists only where an outside-observable outcome exists (process alive, native dialogs, files on disk, browser opened); the plan lists every case without a packaged version and the reason.
- Q: May the `wails dev` E2E tier drive the application in Chromium at the local address `wails dev` serves, with the native window covered only by the packaged-binary cases? → A: Yes; the `wails dev` tier drives Chromium at the local `wails dev` address (same frontend, real backend, real bridge, no mock); the native window is exercised by the packaged-binary cases and the developer walkthrough.
...
4. **Given** the repository after this feature, **When** test files are listed, **Then** none is under `frontend/src`, `frontend/public` or a Go production package, except the white-box exceptions listed in the plan, each with a written reason; and every test sits in the root its dependencies dictate (unit: no disk, database, rendered UI or bridge; integration: real temporary files or database, or rendered React with the store; E2E: the real application).
...
8. **Given** the real-backend suite, **When** it runs, **Then** it covers: activating a preview link and keeping the bridge; a close request before the frontend is ready; failed startup then Retry then close; an isolated settings rejection; a lost cancellation; a late bridge completion. Every case runs on `wails dev` as a blocking check in the release workflow on each target platform's runner (macOS in this feature), and again against the packaged binary only where an outside-observable outcome exists (the plan lists every case without a packaged version and the reason), where only outside-observable outcomes are asserted (process alive, native dialogs, files on disk, system-browser opens) through OS-level automation with no hook in the binary.
...
7. **Given** CI, **When** a push arrives, **Then** an Ubuntu runner runs `scripts/verify --skip e2e` as a blocking check; **and When** a version tag (`v*`) is pushed on `app_version_1_codebase`, or the workflow is dispatched by hand as a dry run, **Then** each target platform's runner runs plain `scripts/verify` including the real-backend E2E stage (the `wails dev` suite and the packaged-binary cases) and builds that platform's artifact only on success; the release workflow exists for at least macOS; the artifact is unsigned in this feature.
...
- A CI runner has no WebKit: the push workflow's Ubuntu runner runs the five stages that need no screen; the E2E stage runs in the release workflow on each target platform's own runner.
...
- A packaged-binary case needs to read the screen inside the webview: it is rewritten to assert an outside-observable outcome, or it runs only on `wails dev` and the plan lists it with the reason; a debugging port, environment-variable seam or test build flavour is never added to the binary.
...
- A `wails dev` case needs a file open or a Save As: the file is opened through the seeded Recents list; a Save As is proven at the bridge-call level with the dialog port replaced at the composition root, or in a packaged-binary case through OS-level automation of the native dialog; the `wails dev` tier never automates a native dialog.
...
- **FR-033**: The real-backend E2E suite shall cover preview-link activation with bridge continuity, a close request before the frontend is ready, failed startup followed by Retry and close, an isolated settings rejection, a lost cancellation and a late bridge completion, running every case on `wails dev` as a blocking check in the release workflow on each target platform's runner (macOS in this feature), and again against the packaged binary only where an outside-observable outcome exists, the plan listing every case without a packaged-binary version and the reason; packaged-binary cases shall assert only what is observable from outside the process (launch, native Open, Save As, close and quit dialogs, files on disk, links opened in the system browser) through OS-level automation, and the binary shall contain no test hook, debugging port or test build flavour. *Source: §5.10.6, RF-D8; clarified 2026-09-08.*
...
- **FR-070**: CI shall have two tiers: on every push an Ubuntu runner shall run `scripts/verify --skip e2e` (Lint, Format check, Build, Unit, Integration) as a blocking check; the release workflow shall run plain `scripts/verify` (all six stages, including the `wails dev` suite and the packaged-binary cases) on the runner of each platform it builds for and shall build that platform's artifact only when they pass; the release workflow shall exist and cover at least macOS; it shall run on a version tag push (`v*`) on `app_version_1_codebase`, publishing the artifact on success, and on manual dispatch from any branch as a dry run that uploads the artifact without publishing; the artifact shall be published unsigned, and code signing and notarisation shall be recorded in the architecture map as an open decision for a later feature. *Source: TL-5, RF-E6; clarified 2026-09-08.*
...
- **FR-073**: The build stage shall validate the actual tagged Wails desktop artifact and the required host toolchain, and the CGO-free check shall be named and scoped to the backend and SQLite rather than claimed for the desktop binary; the Build stage shall fail with a clear message when the host toolchain is missing. *Source: AG-9, RF-E8.*
...
- **Test root**: a directory per test category (unit: no disk, database, rendered UI or bridge; integration: real temporary files or database, or rendered React with the store, no Go process; E2E: the real application through `wails dev`, driven in Chromium at its local address, plus outside-in cases against the packaged binary) with its placement rule and the list of white-box exceptions with reasons.
...
- **Decision record**: a durable product or architecture decision (for example the native window frame, the link policy, the unsigned release artifact) recorded in the architecture map.
...
- **Hosts.** Development and verification happen on macOS; Linux behaviour is verified where a runner exists; Windows is not verified by this feature. The release workflow gains a platform's runner when that platform is verified; in this feature it runs on macOS.
