...
- `app_version_1_codebase`: the release branch; one squash commit per finished feature; version tags `v*` on it start the release workflow.
...
- Q: What starts the release workflow that runs all six stages per platform and builds that platform's binary? → A: A version tag push (`v*`) on `app_version_1_codebase` runs the stages, builds and publishes the artifact; manual dispatch on any branch runs the same stages and uploads the artifact as a dry run without publishing.
...
4. **Given** the repository after this feature, **When** test files are listed, **Then** none is under `frontend/src`, `frontend/public` or a Go production package, except the white-box exceptions listed in the plan (in-package `_test.go` files), each with a written reason; and every test sits in the root its dependencies dictate (unit: no disk, database, rendered UI or bridge; integration: real temporary files or database, or rendered React with the store; E2E: the real application).
...
- A preview link points outside the document's folder, to a non-Markdown local file, or uses a scheme other than `https`/`http`: it is refused with a visible message (see FR-018).
...
- **FR-018**: The shared link handler shall scroll the preview for an in-document anchor; open a local `.md` file that resolves inside the document's folder through the normal open flow; open an `https` or `http` link in the system browser only after the user's explicit activation; and visibly refuse every other target (`mailto`, `file`, `javascript`, `data` and any other scheme, and any local file outside the document's folder) without weakening sanitisation. *Source: APP-1, D11, RF-A12; clarified 2026-09-08.*
...
- **FR-035**: The test suites shall be consolidated according to the audit's keep, rewrite and delete lists (audit §5.4.4): the editor-view suites merged into one, the Settings menu suite folded into the menubar suite, the tab suite slimmed, the App suite rewritten without mocking the shell it tests, the eight behavioural browser journeys ported to the real backend, and the listed bureaucracy suites deleted; before any suite is deleted, the plan shall map every feature 001–003 acceptance scenario to the surviving or ported behaviour test that proves it (or to a packaged-binary walkthrough where no test can reach it), and an unmapped scenario shall be filed as a task. *Source: T-4, T-5, §5.4.4; clarified 2026-09-08.*
...
- **FR-070**: CI shall have two tiers: on every push an Ubuntu runner shall run `scripts/verify --skip e2e` (Lint, Format check, Build, Unit, Integration; the runner installs the Linux Wails toolchain, GTK and WebKitGTK development packages included, and its Build stage links the real Linux desktop artifact, so Build is identical on every host) as a blocking check; the release workflow shall run plain `scripts/verify` (all six stages, the E2E stage being the `wails dev` suite) on the runner of each platform it builds for and shall build that platform's artifact only when they pass; the release workflow shall exist and cover at least macOS; it shall run on a version tag push (`v*`) on `app_version_1_codebase`, publishing the artifact on success as a GitHub Release for that tag with the zipped `.app` attached as a release asset (using the repository's built-in token with contents write permission and no other secret), and on manual dispatch from any branch as a dry run that uploads a workflow artifact without creating a release; the artifact shall be a zipped `.app` bundle built for the runner's own processor architecture, its name stating that architecture (no universal build, no disk image), published unsigned, and code signing and notarisation shall be recorded in the architecture map as an open decision for a later feature. *Source: TL-5, RF-E6; clarified 2026-09-08.*
...
- **FR-085**: The instructions shall state the branch convention: `app_version_1_codebase` as the release branch, `feature/<NNN>-<short-description>` per feature and `feature/<NNN>-<short-description>-<task>` per task; a task branch is optional, used for a task large enough to need several commits, and is squash-merged so the feature branch keeps about one commit per task, while a small task commits straight to the feature branch. *Source: H-7, owner input; clarified 2026-09-08.*
...
- **Test root**: a directory per test category (unit: no disk, database, rendered UI or bridge; integration: real temporary files or database, or rendered React with the store in the test process, not a browser, no Go process; E2E: the real application through `wails dev`, driven in Chromium at its local address, the only browser-driven tier, with profile isolation by config-root redirection; the packaged binary is covered by the developer-host walkthrough) with its placement rule and the list of white-box exceptions with reasons.
...
- **Shutdown request**: a close or quit request with an identity, an acknowledgement state, a cancellation state, a pending-write drain state, a user-confirmation state (when unsaved changes exist), an authorisation state and an acknowledgement deadline of 10 seconds after which the screen counts as lost.
...
- **Hosts.** Development and verification happen on macOS; Linux behaviour is verified where a runner exists; Windows is not verified by this feature. The release workflow gains a platform's runner when that platform is verified; in this feature it runs on macOS. The packaged binary is walked through on the developer's Mac before each release.
