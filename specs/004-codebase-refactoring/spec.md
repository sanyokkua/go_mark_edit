...
7. **Given** a close request, **When** it is handled, **Then** it carries an identity, is acknowledged, can be discovered by a frontend that loads later, can be cancelled with confirmation, and a stale response is rejected; a backend call unanswered for 10 seconds is reported as stuck with Retry and Cancel, a late result is still applied, and a timeout never authorises discarding unsaved work.
...
- **FR-061**: The application shall have one shutdown protocol that owns request identity, acknowledgement, pending-state discovery for a frontend that loads later, cancellation with confirmation, authorisation and stale-response rejection across the backend and the frontend; if a call to the backend stays unanswered for 10 seconds, then the frontend shall report it as stuck with a visible message offering Retry and Cancel, and shall still apply a result that arrives later and withdraw the message. *Source: APP-2, APP-5, RF-C7; clarified 2026-09-08.*
...
- **Test root**: a directory per test category (unit: no disk, database, rendered UI or bridge; integration: real temporary files or database, or rendered React with the store, no Go process; E2E: the real application) with its placement rule and the list of white-box exceptions with reasons.
...
- **Shutdown request**: a close or quit request with an identity, an acknowledgement state, a cancellation state, a pending-write drain state, a user-confirmation state (when unsaved changes exist) and an authorisation state.
...
- **SC-018**: A backend call held for longer than 10 seconds in a test produces the stuck message with Retry and Cancel within 1 second of the bound, and a result released afterwards is applied with the message withdrawn (zero lost results, zero duplicate reports).
