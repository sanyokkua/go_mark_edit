...
18. **Given** a startup failure in the bridge, the model, the settings or the window-ready step, **When** the failure screen appears, **Then** it names the failed step; a bridge failure offers only Quit, the other three offer Retry of that step, and Quit is always available.
...
7. **Given** a close request, **When** it is handled, **Then** it carries an identity, is acknowledged, can be discovered by a frontend that loads later, can be cancelled with confirmation, and a stale response is rejected; a backend call unanswered for 10 seconds is reported as stuck with Retry and Cancel (commands that open a native dialog are user-paced and never reported as stuck), Retry re-sends the same command with the same request identity and the backend answers with the original outcome if that request already completed or is still running (a write is never applied twice), a late result is still applied (also after Cancel, which only dismisses the message), and a timeout never authorises discarding unsaved work.
...
- **Decision record**: a durable product or architecture decision (for example the native window frame, the link policy, the unsigned per-architecture zipped release artifact, the developer-host walkthrough of the packaged binary) recorded in the architecture map.
