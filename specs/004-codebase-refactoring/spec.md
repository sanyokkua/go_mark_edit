...
2. **Given** the E2E stage, **When** it runs, **Then** it starts the real Go application through `wails dev` (real backend, real bridge, no mock) with a disposable profile directory and temporary files, opens those files through a Recents list seeded in that profile, drives it through the screen, and asserts the bytes on disk and the state after a restart.
...
7. **Given** each defect in Story 1, **When** its stable-interface regression test (bridge call or driven screen) is run once on the archived tree, **Then** it fails and the failure is recorded in the plan; on the refactored tree it passes; additional unit or integration tests for the same fix need not run on the archived tree.
8. **Given** the real-backend suite, **When** it runs, **Then** it covers: activating a preview link and keeping the bridge; a close request before the frontend is ready; failed startup then Retry then close; an isolated settings rejection; a lost cancellation; a late bridge completion. Every case runs as a blocking check in the release workflow on each target platform's runner (macOS in this feature), on `wails dev` and again against the packaged binary, where only outside-observable outcomes are asserted (process alive, native dialogs, files on disk, system-browser opens) through OS-level automation with no hook in the binary.
...
- A regression test cannot compile against the archived tree because the refactor changed the interface it calls: the test is moved to the level both trees share (a bridge call or the driven screen); a test that only exists at the new interface is not a regression test in the sense of FR-032.
...
- A packaged-binary case needs to read the screen inside the webview: it is rewritten to assert an outside-observable outcome, or it runs only on `wails dev`; a debugging port, environment-variable seam or test build flavour is never added to the binary.
...
- **Test root**: a directory per test category (unit: no disk, database, rendered UI or bridge; integration: real temporary files or database, or rendered React with the store, no Go process; E2E: the real application through `wails dev`, plus outside-in cases against the packaged binary) with its placement rule and the list of white-box exceptions with reasons.
...
- **SC-001**: Every defect listed in Story 1 has a stable-interface regression test (bridge call or driven screen) whose one-time failure on the archived tree is recorded in the plan and which passes on the refactored tree (nineteen scenarios, zero exceptions without a recorded host reason).
