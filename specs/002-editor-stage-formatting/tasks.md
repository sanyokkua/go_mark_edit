...
**Purpose**: Close the theme-dependent popup and native-window gaps exposed by the fresh T093 walkthrough, then prove that Liquid Glass, Material, and Minimal share the same Editor-stage implementation and behavior while differing only through approved styling tokens and generated editor palettes. Preserve the OS-managed frame, centralized appearance authority, exact action registry, responsive boundaries, deferred controls, historical evidence, and local-only request proof.

- [x] T094 HIGH [US1][US4] Consolidate every Editor-stage popup onto one body-portal, measured viewport-coordinate contract so Liquid Glass styling cannot change popup ownership or geometry per FR-ED-019, FR-ED-023, SC-ED-001, SC-ED-005, ED-VS-05, and Constitution VI (partial)
  - Scope: audit and repair `frontend/src/ui/widgets/EditorContextMenu.tsx/.module.css`, `ShellMenuRow.tsx/.module.css`, `SettingsMenu.tsx/.module.css`, `EditorChrome.tsx/.module.css`, `frontend/src/ui/primitives/ViewMenu.tsx/.module.css`, and the relevant `EditorView` containment styles. Keep one active File/Settings/View/About owner, current opener-focus restoration, Escape/outside dismissal, modal suppression, registry-derived inventories, and all deferred outcomes unchanged.
  - Implementation contract: add failing focused tests before production edits. A right-click context menu must be rendered outside filtered, transformed, clipped, or overflow-hidden ancestors and must use the original `clientX`/`clientY` viewport point. Triggered menus must use the trigger's viewport `getBoundingClientRect`. Measure the rendered rectangle after mount, apply the existing 8px collision margin, clamp both axes, flip above/beside when necessary, and recompute on viewport resize and scroll. Radix and custom portal paths must resolve to the same body-owned coordinate space; do not introduce Liquid-Glass-only offsets or theme branches.
  - Acceptance criteria: at 1280, 768, and 375 pixels, in all six palettes, File, Settings, View, About, toolbar overflow, and editor context menus open adjacent to their actual trigger/pointer, remain fully reachable and viewport-contained, preserve long-label and separator row widths, and never let a filtered Glass ancestor alter their coordinates. Focus, selection capture, action identity, disabled Format/Compact/Command palette outcomes, and no page-level horizontal scrolling remain unchanged.
  - Proof: extend `EditorContextMenu.test.tsx`, `ShellMenuRow.test.tsx`, `SettingsMenu.test.tsx`, `ViewMenu.test.tsx`, and `EditorChrome.test.tsx` with filtered-ancestor portal, corner-placement, resize/scroll, hit-test, owner, focus, and long-label cases; extend `frontend/e2e/editor-stage.test.ts` with actual pointer/trigger bounding-box assertions for every width/theme/mode combination. Run the affected focused suites before and after the source change and retain any real-bridge observations under `specs/002-editor-stage-formatting/evidence/`.

- [x] T095 HIGH [US1][US2][US3][US4] Enforce theme implementation parity so the six appearance combinations change only centralized styling and generated editor palette data, never structure, ownership, geometry, action behavior, focus, or responsive rules per FR-ED-023, FR-ED-024, SC-ED-001, SC-ED-002, SC-ED-003, SC-ED-005, and Constitution VI (partial)
  - Scope: audit `frontend/src/logic/theme/theme.ts`, `frontend/src/ui/styles/tokens.css`, `frontend/src/ui/styles/tokens.test.ts`, `frontend/src/logic/theme/theme.test.ts`, `frontend/src/ui/widgets/{ShellMenuRow,SettingsMenu,ViewMenu,EditorChrome,EditorContextMenu,EditorView}.*`, `frontend/src/ui/components/monacoThemes.ts`, and the existing appearance/editor-stage E2E matrices. Generated `generatedEditorThemes.ts` remains generator-owned and must not be hand-edited.
  - Implementation contract: add failing parity tests before any repair. Define a theme-independent semantic signature for visible Editor-stage inventories, roles, action IDs, disabled/deferred states, popup ownership, focus restoration, responsive relocation, page/tab-strip scroll boundaries, and formatting outcomes. Sequentially switch through Glass, Material, and Minimal in Light and Dark at 1280, 768, and 375, and assert that the signature and authoritative interaction results remain identical while only token-backed visual values and the generated Monaco theme vary. Remove or refactor any discovered theme/mode behavior branch; keep `data-theme`/`data-mode` on the document root as the sole appearance selector and preserve in-place Monaco identity, caret/selection, scroll, undo history, and acknowledged Go/settings state.
  - Acceptance criteria: every required menu, overflow, tooltip, context menu, toolbar group, sidebar control, arrangement control, formatting action, and deferred action has the same DOM/registry identity, semantic name, availability, keyboard/pointer behavior, geometry contract, and focus behavior in all six palettes. Liquid Glass blur, Material/Minimal no-blur, colors, fonts, radii, shadows, and Monaco colors remain token/generated styling differences only; no literal color, second appearance authority, hidden lifecycle, or network path is introduced.
  - Proof: add focused semantic-signature and theme-transition coverage to the named theme/component suites; expand `frontend/e2e/appearance.test.ts` and `frontend/e2e/editor-stage.test.ts` so each palette opens the same popup/context/overflow journeys and records matching bounds, hit targets, focus, root attributes, page scroll, contained tab scroll, formatting/undo, and deferred no-op results. Inspect the rendered Glass Light/Dark and Material/Minimal surfaces in the real Wails bridge before accepting any visual baseline change.

- [x] T096 HIGH [US1][US3] Enable the ordinary macOS native zoom/fullscreen control without replacing the OS-managed frame per FR-ED-001, FR-ED-004, FR-ED-026, SC-ED-006, ED-VS-04/06, and Constitution V/VI/VII (partial)
  - Scope: add a failing native-options assertion in `main_test.go`, then update `main.go` and the matching `cmd/native-evidence/main_native_evidence.go` harness configuration if it is used for the current-host proof. Keep `Frameless: false`, `DisableResize: false`, the existing minimum size, startup/close lifecycle, Wails runtime fullscreen adapter, F11 binding, and native titlebar ownership unchanged.
  - Implementation contract: provide explicit Wails macOS options with native zoom enabled (`DisableZoom: false`) so a resizable framed window exposes an active green traffic-light zoom/fullscreen control. Do not set the application to start fullscreen, add a custom button or drag region, implement private geometry calls, or enable WebView content fullscreen as a substitute for the native control. Verify that View → Full screen and F11 still route through the existing adapter and that non-macOS builds retain their existing options.
  - Acceptance criteria: the freshly built macOS app shows an enabled green native control; operating it enters and leaves the host-supported native zoom/fullscreen state while preserving the prior frame, position, and size; native titlebar movement, edge/corner resize, close, and the in-app fullscreen action remain OS/runtime-owned. The native evidence harness, when used, has the same capability contract and cannot report a false disabled-control result.
  - Proof: run the new Go option test and relevant adapter tests before and after the change, then verify the packaged current-host binary manually with the green control, View → Full screen, F11, restore, movement, resize, and close. Retain host limitations honestly and do not add synthetic native-control proof.

- [x] T097 HIGH Complete the T093 real-control and current-host release evidence after T094–T096, without rewriting historical records or promoting unresolved behavior per T093, SC-ED-005 through SC-ED-007, ED-LIVE-001 through ED-LIVE-005, and Constitution VII (partial)
  - Dependency/order: execute only after T094, T095, and T096 focused tests and implementation are green. Capture a trustworthy current baseline before the first implementation edit; compare fresh gate results with it and treat any no-analysis nonzero gate as `UNRELIABLE`.
  - Real development-bridge proof: operate the actual Wails controls—not synthetic DOM events—at 1280, 768, and 375 across Glass/Material/Minimal × Light/Dark. Exercise File, Settings, View, About, toolbar overflow, editor right-click context menu at known pointer coordinates, formatting and undo, sidebar, arrangement, visual tabs, and every deferred Image/Format/Compact/Lint/Assistant/File/tab boundary. Record root `data-theme`/`data-mode`, popup and hit-test rectangles, focus restoration, page versus tab-strip scroll widths, computed blur/token behavior, and local-only request observations.
  - Current-host package proof: build a fresh current-host app and operate the OS-managed frame, native movement, manual edge/corner resize, enabled green zoom/fullscreen, View/F11 fullscreen, restore, and close together with the corrected popup/theme journeys. State Windows/Linux and package-minimum-size limits explicitly rather than inferring them from macOS or the mock bridge.
  - Acceptance criteria: affected focused tests, formatting/type/lint/unit/architecture/build/feature gates, and the required real walkthrough pass without weakened locators, retries, parallelism, screenshot tolerance, configuration, or test scope. Append unabridged raw output, screenshots, and a factual summary only under `specs/002-editor-stage-formatting/evidence/`; preserve Phase 16–21 records and the prior local-only proof unchanged. T093 may be checked only after every named real-bridge and packaged-app criterion is observed successfully.
