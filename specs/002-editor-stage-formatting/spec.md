...
- A document has more than 1,000 lint findings: the later tidy/lint slice retains the original exact-count
  and decoration-cap requirement, but this slice has no lint findings, count, decorations, or lint state and
  MUST not claim that deferred behavior is active.
...
At 375 pixels, the tab strip may use contained horizontal scrolling for visual reachability only;
this feature specification governs behavior. The completed
...
A deferred rule remains a named downstream boundary and is not represented by a fake successful
command.
...
- **FR-ED-003**: The Settings menu MUST expose the original mockup inventory: theme swatches,
  ...
  and no Assistant settings group may appear. Default open mode (Reading/Editor) is a file-lifecycle
  setting and is distinct from the live Editor/Split/Preview view arrangement; Markdown Standard
  (Minimal/GFM/Full) is a deferred renderer inventory and is distinct from the active marker preferences.
...
- **FR-ED-010**: The controls bar MUST expose Format, Compact, and Lint using the original labels and
  ...
  operation until the later tidy-markdown slice owns those behaviors.
...
- **FR-ED-012**: Formatting actions MUST operate only on the selected range or current line and MUST
  ...
  separate component path or become a second canonical document store. The transformation work and
  replacement span MUST be bounded by the selected range or current-line length plus required line-boundary
  bytes, independent of total document length; no whole-document parse or copy is part of this slice.
...
- **FR-ED-014**: Heading actions MUST add the requested ATX heading level to a paragraph, replace a
  different existing level, and remove the marker when the requested level is already active. They MUST
  NOT convert Setext headings or invent a Setext H3 fallback; the existing heading-style preference remains
  a separate acknowledged setting and does not change these ATX action semantics.
...
- **FR-ED-018**: The existing long-operation gate MUST not be acquired by the deferred Format, Compact,
  ...
  partial document changes; the full busy, cancellation, failure, and release contract is deferred.
...
- **SC-ED-001**: In the automated responsive matrix, all 18 combinations of 3 widths (1280, 768, and 375) and 6 theme/appearance palettes render the in-scope chrome without horizontal clipping or an
  unreachable control.
...
- For this slice, the consumed implemented-action set is appearance persistence, Markdown marker
  preferences, editor display settings, Editor/Split/Preview arrangement, left-sidebar visibility,
  full-screen, local About/version, and native clipboard/undo roles; no other existing or future action
  is implicitly promoted into scope.
