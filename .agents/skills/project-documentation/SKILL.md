...
version: 2.0.0
description: >
  Generate comprehensive, accurate documentation of the GoMarkEdit **code/implementation** from its
  actual contents — architecture, inputs/outputs, integrations, data flow, business logic, data
  ...
  project-navigator). Boundaries: **never edits `specification/**` (the frozen source of truth) and
  defers all normative behavior to it**; never invents endpoints/fields/behavior (marks gaps
  "TODO: confirm"); never pastes secret values into docs; never commits or pushes without the user.
tags: [documentation, architecture, service-docs, data-flow, business-logic, integrations, data-contracts, configuration, repository, gomarkedit, wails, go, typescript]
...
`specification/**` is the **frozen source of truth** for GoMarkEdit's normative behavior. This skill
documents the **code/implementation** — how it is actually built — into `docs/` (committed) or
`.agent-docs/` (scratch). It **never** edits, "improves", duplicates, or contradicts `specification/**`.
When you need to state *what the system must do*, **cite and link the spec** (e.g.
`specification/02_Architecture/02_BACKEND_GO.md#…`) rather than restating it as if this doc were
authoritative; if code and spec disagree, that is a finding to flag ("TODO: confirm — code diverges from
spec"), not something to paper over. Normative requirements live in the spec; implementation notes,
architecture-as-built, and how-to-run live in the docs you write here. New normative decisions are ADRs
(`docs/adr/`, `0013+`), not edits to the spec.
...
- [ ] **No file under `specification/**` was created, edited, or deleted** — the spec is frozen;
      normative claims cite/link it instead of restating it as authoritative.
