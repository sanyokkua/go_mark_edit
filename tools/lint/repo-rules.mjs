...
const textExtensions = new Set(['.cjs', '.css', '.go', '.js', '.json', '.md', '.mjs', '.ts', '.tsx']);
...
function pathCandidate(candidate) {
    ...
    if (/^(?:README|AGENTS|CLAUDE)\.md$|^\.golangci\.yml$|^go\.mod$|^justfile$|^package\.json$/.test(cleaned))
        return cleaned;
    return null;
}
...
const findings = [];
findings.push(...(await scanTextFiles(root, (path) => isSourceFile(path) || isTestFile(path), 'L22')));
