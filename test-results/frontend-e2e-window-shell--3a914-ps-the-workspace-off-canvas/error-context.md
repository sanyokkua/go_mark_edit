...
- Name: frontend/e2e/window-shell.test.ts >> T039 native minimum frame rounding keeps the workspace off-canvas
- Location: frontend/e2e/window-shell.test.ts:368:1
...
```ts
  272 |             .evaluate((element) =>
  273 |               getComputedStyle(element).getPropertyValue('--dur-base').trim(),
  274 |             ),
  275 |         )
  276 |         .toBe('0ms');
  277 |       await page.getByRole('button', { name: 'Close' }).click();
  278 | 
  279 |       const workspace = page.getByRole('complementary', { name: 'Workspace' });
  280 |       await expect(workspace).toBeVisible();
  281 |       await expect
  282 |         .poll(async () => (await workspace.boundingBox())?.width)
  283 |         .toBe(width === 375 ? 230 : width === 768 ? 46 : 256);
  284 |       await expect(
  285 |         page.getByRole('main', { name: 'Document area' }),
  286 |       ).toBeVisible();
  287 |       await expectFutureSurfacesAbsent(page);
  288 |       await expect
  289 |         .poll(() =>
  290 |           page.evaluate(
