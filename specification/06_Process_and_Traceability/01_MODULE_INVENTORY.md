...
The authoritative list of every shippable module. Every story's `modules:` front-matter MUST name a
...
## Stage 3 — LLM assistant modules

These modules are added in Stage 3 (`00_Foundation/06_IMPLEMENTATION_STAGES.md`). They consume the
reserved seams from Stages 1–2 (F1–F9) and must not exist earlier. See
...
- **Stages 1–2** backend: **15** packages + `main.go` (incl. `internal/appmodel`). Frontend: **13** module folders.
