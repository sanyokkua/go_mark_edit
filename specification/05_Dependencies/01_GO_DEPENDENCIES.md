...
**Cross-references:** `00_Foundation/04_DESIGN_DECISIONS.md` (DD-03, DD-08, DD-10, DD-13, DD-33), `06_Process_and_Traceability/01_MODULE_INVENTORY.md`, `04_Build_and_Release/01_BUILD_MATRIX.md`, `05_Dependencies/03_DEPENDENCY_POLICY.md`
...
offline-first Markdown editor, so Stages 1–2 carry no HTTP client, tokenizer, or single-instance-lock
dependencies (the Stage-3 provider client is built on the standard library `net/http`; see §3).
...
| `resty.dev/v3` (Resty HTTP client) | The only outbound traffic GoMarkEdit ever originates is the user-invoked Stage-3 LLM inference (DD-32 as revised), and that provider client is built on the standard library `net/http` (`02_Architecture/08_LLM_INTEGRATION.md` `#provider-abstraction`) — no third-party HTTP client belongs in the backend at any stage. Document-referenced remote assets are governed by the frontend content policy and the guarded AssetServer, not a Go HTTP client. |
| `github.com/pkoukk/tiktoken-go` + `tiktoken-go-loader` | Not carried in Stages 1–2 (no LLM, no tokenization). The Stage-3 offline tokenizer (`internal/llm/tokenizer`) needs an embedded estimator; if an embedded-BPE library is chosen, it is added by that story under the dependency policy (offline, no CGO, MIT-compatible — `05_Dependencies/03_DEPENDENCY_POLICY.md`) and never appears before Stage 3. |
