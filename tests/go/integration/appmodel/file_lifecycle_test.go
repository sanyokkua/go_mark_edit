...
func TestNewDocumentDefaultsAndNoWrite(t *testing.T) {
	...
	if after.Snapshot.ActiveDocumentID != documentID {
		t.Fatalf("New acknowledgement/state identity = %q/%q, want the new active document", outcome.Data.DocumentID, after.Snapshot.ActiveDocumentID)
	}
...
// whenever the acknowledged default is Reading, and since that setting
...
func TestNewDocumentRefusesStaleOrFortyFirst(t *testing.T) {
	...
	staleService := NewAppModelServiceForHost(WithEmitter(staleEmitter))
	...
	if len(afterFortyFirst.Snapshot.Documents) != 40 || len(afterFortyFirst.Snapshot.OrderedDocumentIDs) != 40 || len(emitter.patches) != patchesBefore {
