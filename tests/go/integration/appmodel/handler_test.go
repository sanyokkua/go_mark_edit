...
func TestHandlerReturnsTypedResultsAndRecoversPanics(t *testing.T) {
	...
	cases := []struct {
		...
		// . Three inputs: the receiver, the document id and the token — and
...
func TestOpenCancellationHasNoMutation(t *testing.T) {
	...
	service := NewAppModelServiceForHost(WithEmitter(emitter), WithDialogs(cancellationDialog{}, nil))
...
// : the release is in-memory only, which is why it takes no context.
