...
func TestApplicationHandlerExposesTypedRecoveringRetryStartup(t *testing.T) {
	...
	if method.Type.NumIn() != 2 || method.Type.NumOut() != 1 || method.Type.Out(0) != reflect.TypeOf(apperr.VoidResult{}) {
		t.Fatalf("RetryStartup signature = %s, want bridge.Request first and one apperr.VoidResult", method.Type)
	}
