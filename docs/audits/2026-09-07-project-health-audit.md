...
**Revision:** 2.2 (agent guidance and mechanical enforcement review), 8 September 2026. Revision 2.1 is preserved as
[audit-revision-2.1-before-agent-review.md](2026-09-07-project-health-evidence/audit-revision-2.1-before-agent-review.md).
The owner-reviewed Revision 2 is preserved
...
criteria (V1–V8) · §3 What was verified and the fact-check of revision 1 · §4 Root causes (RC-1…RC-9) · §5 Findings
...
spec drift; 5.10 preview-link/startup/quit incident; 5.11 agent guidance and mechanical enforcement) · §6 Refactoring
scope (epics A–F, non-goals,
...
**Agent/configuration extension, also 8 September.** Revision 2.2 reviews `.agents`, `.claude`, root instructions,
workflow support and project enforcement configs against the owner's latest direction: KISS, DRY, SOLID, autonomous
engineering judgment, and mechanical enforcement of mechanical rules. §5.11 gives the findings and the intended
instruction/tool boundary. It also corrects specific earlier config claims and revises Epic F: an arbitrary prose
line limit, a documentation-test gate and mandatory Spec Kit ceremony would repeat the problem. This remains an audit;
live agent instructions, skills, configs and application code have not been rewritten.
...
**V8 — Agent guidance explains intent; tools enforce mechanical rules.** KISS, DRY and SOLID guide responsibility,
reuse and dependency decisions: find the existing owner of a behavior, improve that owner, and update its actual
consumers. Use small cohesive components and narrow interfaces at real boundaries; abstractions need current use.
Follow current idioms supported by the declared language/library versions and prefer documented simplifications.
Formatting, syntax, concrete import restrictions and other reliable static checks belong in the shared tooling, not
duplicated prose. Tests demonstrate meaningful behavior. Agent instructions retain the judgment tools cannot supply:
product intent, correct ownership, sensible reuse, clear contracts and risk-aware verification. Routine reversible
decisions within the authorized task do not require a new planning phase or permission request. These principles do
not require novelty, an interface for every type, or a universal component with every possible option.
...
### 3.4 Agent/configuration review evidence (Revision 2.2)

Source remains `883fd05`. The previous report is preserved as `audit-revision-2.1-before-agent-review.md` (286,598 bytes,
SHA-256 `30c58d513980400743674104db170db80491cec4259f1110a0ae6a4242f2a45a`). The three preexisting runtime mode diffs,
untracked audit directory and newly present `.idea/` at this turn's start were left in place. Only audit/evidence files
were edited. No live instruction, configuration, application code or workflow state was changed.

| Check | Result / limits |
| --- | --- |
| Complete agent-tree inventory | 37 regular files plus five directory symlinks: every `.agents` body reviewed, every Claude core body compared, links resolved. This is storage/invocation inventory, not a claim all bodies load on every turn. |
| Generated ownership | Twenty core client files and twelve shared support files match their manifests; three extension metadata hashes match the separate registry. Different client invocation forms are legitimate. No integration refresh or launcher was invoked. |
| Project config and policy review | Root instructions, constitution, workflow/templates/extensions, lint/format/compiler/test/build/hook/CI configs traced to actual owners. Source risks are labelled as such, including Ralph and CGO artifact scope. |
| Effective normal ESLint configuration for `src/App.tsx` | Read-only calculation, exit 0. Parser options have no type-project setup; promise and restricted-import rules are absent. Hook rules exist; exhaustive-deps and refresh checks are warnings. This is configuration inspection, not a fresh full lint run. |
| Isolated golangci/gofmt diagnostic | Unchanged copied `.golangci.yml`, local golangci-lint 2.13.2: deliberately unformatted temporary input exits 1 with one gofmt finding; formatted control exits 0 with zero issues. Both definitive runs have empty stderr and a 25-second external limit. CI's 2.12.2 was not tested. Coverage differs from the tracked-file gofmt recipe. |
| Recommendations for current idioms/tools | Checked against primary Go, TypeScript, typescript-eslint, React and Stylelint documentation; source links retained. No tool upgrade or proposed config was applied. |
| Audit self-corrections | Frontend Prettier config exists; the `net` import ban is alias-independent; generated skill wrappers differ meaningfully; incomplete client registration is not proof all fallback paths fail; untagged CGO compilation is not native-build certification. |

Full gate results in earlier revisions remain historical. The new diagnostic neither tests the application nor
certifies all current lint/format stages. Ralph, GitHub issue creation, bridge handoff and reconciliation were not
executed. The review checks behavior of their instructions/source without attributing historical app defects to an
unobserved invocation. The retained incident section and appendices remain unchanged.
...
| PR-1…PR-5 | P2 | Conflicting authority pointers; dead command references; `AGENTS.md` war stories; incomplete client registration of mandatory hooks; two live workflows | source | 5.7 |
| AG-4 | P1 before reuse | Ralph ignores requested free-form scope, widens permissions and accepts completion before checking child failure | source; launcher not executed | 5.11 |
| AG-1…AG-3, AG-5…AG-10 | P2 | Excessive ceremony, missing reuse guidance, inconsistent procedure state, mechanical coverage gaps, toolchain drift and misleading CGO certification | source/config; bounded formatting diagnostic | 5.11 |
...
`baseline`, `verify`. Fourteen one-line aliases. Overlap: `go-format-check` and golangci's configured gofmt
(`.golangci.yml:20–28`; confirmed for installed v2.13.2), but their generated/vendor exclusions differ. Consolidate
formatting ownership without dropping coverage. Other overlap: `go-vet` (`govet` enabled, `:6`), `e2e-test`
(alias of `verify-ui` "named for the verb the specification uses"), `cgo-free-check` (overlaps the source assertions in
`main_test.go:590–638`; neither certifies the native artifact), `migration-immutability-check` (10 lines of
...
`.github/workflows/*.yml`, `.agents/**`, `.specify/**`. The frontend does have `.prettierrc.json` (semicolons, single
quotes, trailing commas); the gap is repository-wide coverage and ownership, not an absent frontend config. The pre-commit hook
...
| Network imports / single-instance-lock syntax | `:301–364` | `net` and `net/*` are rejected by import path, including aliased imports and `net.Dialer`. The selector checks for `Flock` / `os.O_EXCL` are name-based, however, and exclusive file creation alone does not establish a forbidden single-instance lock. Retain only checks that match a real supported invariant. |
...
| CGO-free by grepping `go.mod`/`main.go`/`justfile`/`wails.json` | `main_test.go:590–638`; `justfile:86–87` | Source literals and an untagged Go build do not prove a runnable native desktop is CGO-free. Preserve pure-Go SQLite; correct the native-build scope (§5.11). |
...
| Prohibited network calls | `archtest.mjs:194`, `package.json` `postbuild`, `architecture_test.go:301` | V1 needs packaged-asset verification and a cold-start offline journey. Retain a network restriction only for an explicitly supported boundary; one scanner per language is not itself a requirement or proof of offline usability. |
...
`docs/delivery/work/baselines/` with a hard-coded exception table (`evidence_id.sh:33–40`). Historical `master`'s
`CLAUDE.md` describes the July regime (`specification/`, `docs/stories/`, `just trace`); the current checkout's five-line
`CLAUDE.md` imports `AGENTS.md` and is already a useful shared-policy pointer. An old branch's instructions must not be
counted as another active root in this checkout.
...
literals, no-network grep, migrations add-only, an untagged CGO-free build, gen-check drift, UNRELIABLE-baseline refusal.
The untagged build does not certify the native desktop artifact (§5.11). Everything else in that list of instructions is
advisory.

**PR-4 Spec Kit extensions declare mandatory hooks with incomplete client registration.** `.specify/extensions.yml` installs
...
symlinks. The ten core `speckit-*` pairs differ in Claude-specific metadata **and invocation syntax** (`/speckit-…`
versus `$speckit-…`), and match their separate integration manifests. These are intentional generated wrappers, not
policy forks to merge by hand. Missing registered extension counterparts establish inconsistent packaging; this
source review alone does not prove every Claude invocation has no possible fallback (§5.11).
...
cannot execute the legacy loop as written because its validators are absent. The Spec Kit extension path additionally
requires capability/registration verification on each client before it can be relied on; missing counterparts should
not be promoted into a claim that all possible execution paths were tested and failed.
...
### 5.11 Agent guidance and project configs: enforce the right things in the right place

**Owner clarification:** agent guidance should preserve intent and support autonomous engineering; code style and
other reliable mechanical checks belong in tools. KISS, DRY and SOLID mean useful shared ownership and simple,
maintainable code. This extends PR-1…PR-5: repairing broken command names alone would leave the excessive process and
missing design guidance intact.

#### 5.11.1 Actual inventory and ownership

The inventory contains **27 regular files under `.agents`**: five legacy command bodies, five pointer skills, ten
core generated Spec Kit skills and seven extension/alias skills. `.claude` contains ten core generated skills and
five valid legacy directory symlinks. Neither tree has a live `rules/` directory; `.claude/commands`, hooks and settings
files are also absent. Current `CLAUDE.md` is a five-line import of `AGENTS.md`. These facts distinguish current policy
from old branch inventories and avoid inventing rule files that no client loads.

All twenty core generated skills and twelve shared support files match their integration manifests. The ten core
skill pairs share substantive workflow text but legitimately differ in client metadata and `$speckit-…` versus
`/speckit-…` invocation. **Keep one owner for project meaning, while preserving required client packaging.** Do not
symlink or hand-edit the generated pairs, copy seven extension skills just to complete a directory count, or alter
manifest hashes to conceal drift. There is no need to maintain a second project-style policy in `CLAUDE.md`.

Detailed file disposition and references are preserved in
[investigation-agent-assets.md](2026-09-07-project-health-evidence/investigation-agent-assets.md),
[investigation-agent-governance.md](2026-09-07-project-health-evidence/investigation-agent-governance.md), and
[investigation-mechanical-enforcement.md](2026-09-07-project-health-evidence/investigation-mechanical-enforcement.md).
These are source/configuration reviews; unused launchers were not executed and no claim is made that every installed
workflow caused the historical implementation failures.

#### 5.11.2 Findings beyond stale references

**AG-1 — Routine work is constrained by process rules unrelated to the product (P2).** `AGENTS.md:14–17,36–40` says to
stop on any unspecified decision, while `:27,90–105` demands one task/branch/commit and a task branch before touching a
file. The legacy planner caps a story at five rules (`.agents/commands/plan-story.md:38–40`); the implementer must stop
on missing template rows and unlisted details. `speckit-implement/SKILL.md:59–100` asks for permission on open checklist
items before reading the implementation context. These rules can block a fully authorized fix to a common owner or
force an artificial split across its consumers. Retain proportionate planning, reviewable changes and decisions for
real product ambiguity; remove routine approval, path and task-cardinality barriers. A missing check limits what can
be claimed verified; it need not halt unrelated diagnosis or authorized repair.

**AG-2 — Templates manufacture duplicate authority and low-value tests (P2).** Legacy `plan-story.md:52–83,112–168`
copies full source clauses/tables/examples, checks copy line counts, injects architecture rules by globs and names a
test for each rule. `DOD_TEMPLATE.md:27–40` also requires tests touching every changed source file. The constitution
repeats exact requirement/task/proving-test mappings (`:36–43,149–163`), while the generated tasks template permits
logical groups and optional tests. This duplicates authority and rewards per-rule bookkeeping. Its contribution to
the bad assertions/comments in T-3/DOC-1 is a process-pressure inference, not traced authorship: several of the same
skills explicitly forbid source-text and mock-self tests, and none instructs authors to put task diaries in production
comments. Keep requirements in their authority and map meaningful scenarios to evidence as needed;
there is no required one-to-one relationship between a clause, task, file and test. Retiring meaningless tests must
be allowed when their purpose is evaluated; never delete a real failing behavioral regression merely to get green.

**AG-3 — The instructions name architecture folders more clearly than shared responsibilities (P2).** Root guidance
explains adapters, Redux and tokens, but omits an equally concrete instruction to inspect and extend the existing
component/policy owner before creating a private copy. The constitution's UI principle names registries and tokens
without the shared popup/frame/lifecycle ownership requested by the owner. `structure.md:78,86,91` prescribes folders,
handler/service/repository growth and a second-caller heuristic. The question should be “who already owns this
behavior and who consumes it?”, not “which new files does this task's template prescribe?” Apply that to menus,
buttons, panes, commands, settings, persistence and lifecycle alike. Genuine engine differences remain inputs or
separate responsibilities; similar text alone does not justify one universal abstraction.

**AG-4 — Optional automation can disregard scope and accept unreliable completion (P1 before reuse).**
`speckit-ralph-run/SKILL.md:18–30,46–57` explicitly ignores free-form requests such as “Implement US1”, then continues
launching the task loop; the protected-branch check warns and continues. The launcher uses broad permission flags
(`ralph-loop.sh:396,402,471,517`, including `--yolo`, `--dangerously-skip-permissions` and `--sandbox danger-full-access`).
At `:655–660` it accepts a COMPLETE marker and breaks **before** handling a nonzero agent exit at `:663–680`.
Thus a failing child that prints the marker can be classified complete by source control flow. This is not a
reproduced historical failure, and those flags do not prove the current host's permissions were bypassed.
The associated defaults also promote broad staging. If unused, retire this route and its hooks. If retained, preserve
user scope and normal host permissions, check execution failure first, verify task outcomes, and stage only owned
changes. “Autonomous” must not mean discarding the requested scope or treating an agent's completion sentence as proof.

**AG-5 — Procedures disagree about their own state and evidence lifecycle (P2).** Build guidance says to preserve
baseline evidence (`build-story.md:41–45`), while `reconcile.md:69–73` orders deletion of phase baselines. Bridge
metadata promises one executor state while
the actual guard only blocks implementation during `executing`; no active handoff JSON is present now. Repeated
mandatory memory-loader calls print all memory rather than selecting relevant context. The task-to-issues flow uses
task IDs such as `T001` in repository-wide deduplication even though IDs repeat per feature. These are operational
contracts, not formatting problems. Keep only useful maintained procedures, give each state/output one owner, key
cross-feature identity correctly, and retire hooks/references together with unused commands. PR-4/PR-5 now distinguish
inconsistent registration from an unproved claim that every possible client fallback fails.

**AG-6 — Static checks cover proxies for rules and miss actual boundaries (P2).** Architecture enforcement is split
between Go tests, two ESLint configs, Node scanners, source-grepping tests and recipes (TL-6). Type names ending in
`Handler`, constructors starting `New`, parameter names and `os.O_EXCL` are proxies: they can reject valid alternatives
or miss equivalent code. The network import ban itself uses import paths and is not bypassed by aliasing `net`; the
earlier TL-6 claim was corrected. Component → store/registry dependencies and concrete layout/style ownership still
escape relevant enforcement. Move precise useful boundaries into Lint, use parser/type/import information appropriate
to the constraint, and remove obsolete name/shape/paperwork rules. Moving a useless test into a custom linter does not
make it useful.

**AG-7 — Available tools are not configured to provide the intended frontend assurance (P2).**
`eslint.config.js:13` uses the untyped TypeScript recommended preset without project type information. Consequently
type-dependent checks such as unhandled promise usage are not provided by that configuration. The architecture
config is separate from ordinary Lint, and its i18n checks cover only selected literal forms. `tsconfig.json:13–23`
uses legacy `moduleResolution: "Node"` while Vite bundles the frontend, and includes UI and browser tests in the same
project. Review actual compiler/test/tool boundaries before enabling typed checks and bundler resolution; do not
blindly apply one tsconfig to Node scripts, Jest and the webview. Existing React Hooks rules are useful and should be
checked in their effective installed form before adding duplicates. A promise linter does not prove cancellation,
progress or native shutdown liveness; APP-1…APP-5 still require behavioral verification. The read-only effective
configuration capture confirms promise/import-boundary rules are absent from normal Lint while hook correctness
rules are present; some diagnostics, including exhaustive hook dependencies, are warnings rather than gate errors.

The test guards likewise do not match the prose: `AGENTS.md` forbids committed focused tests, but Playwright has no
`forbidOnly` setting or equivalent configured lint rule, and `justfile:60` passes `--passWithNoTests` to Jest. An
unexpectedly empty required category or a committed `.only` must fail in the runner/Lint stage. This should not forbid
running a useful targeted subset locally while diagnosing a problem. No empty/focused-suite reproduction was run here.

**AG-8 — Mechanical coverage and toolchain versions have multiple owners (P2).** Format is frontend-rooted, staged
hook patterns differ from verification, and scripts/root configuration/agent Markdown are missed (TL-3). Main ESLint's
explicit ignores also omit reports/results that Prettier excludes; generated report JavaScript can enter the lint
walk if present. That is a configuration exposure, not a reproduced report-directory failure. The frontend
does have a Prettier config; its absence was an audit error now corrected. CI installs Wails CLI **2.12.0**
(`.github/workflows/main.yml:41`) while `go.mod:8` selects Wails **2.15.0**. CI uses Node 22 while the audited host used
24, with no shared Node declaration; `wails.json:5` uses `npm install` where setup/CI use `npm ci`. These do not by
themselves prove a failed build, but make regeneration and validation depend on the entry point. Use one declared
toolchain/dependency installation policy and one invocation graph across local, hooks and CI. Preserve lockfiles;
“latest patterns” is not a reason to resolve a new dependency graph on each build.

**AG-9 — A green CGO check cannot satisfy the written native-build requirement (P2).** The constitution says the shipped
Wails binary is CGO-free (`constitution.md:80`; `rules.md:533–547`). `justfile:86–87` instead runs an **untagged**
`CGO_ENABLED=0 go build ./...`. On macOS/Linux, Wails v2.15 selects `internal/app/app_default_unix.go:1,16–17`, whose
CreateApp reports that the correct build tags are missing. The real production path selects the desktop frontend;
Darwin's frontend imports C and links Cocoa/WebKit (`internal/frontend/desktop/darwin/frontend.go:6–16`). The Wails
builder adds the production tag and enables CGO on non-Windows builds (`internal/app/app_production.go`;
`pkg/commands/build/base.go:225–226,305,327–329`). The source-literal test does not inspect those dependency paths. **Preserve CGO-free
SQLite/backend portability; stop claiming the runnable native desktop is CGO-free.** Correct the active contract and
validate the actual production build, recording the original contradictory requirement without silently rewriting
the archived specification. This is a concrete case of a successful check proving a different artifact from the one
the instruction promises.

**AG-10 — Blanket “best practices” and the audit's earlier quotas can recreate the same drift (P2).** Neither a linter
nor a file-size threshold establishes KISS, DRY, SOLID, correct abstraction or a useful public API. Requiring every
feature to add layers, every function to have an interface, every component to be memoized, or every file to contain
one syntactic category would replace engineering judgment with another checklist. Revision 2.2 therefore replaces
RF-F2's 100-line cap/new documentation gate, RF-F3's compulsory Spec Kit workflow, and RF-B13's rigid file-category
criterion with useful guidance and cohesive ownership. This is an explicit response to the latest owner direction;
earlier revisions remain preserved. Prefer supported current idioms and standard library/framework facilities when
they simplify the actual work; retain explicit lifecycle/side-effect control where the product needs it.

#### 5.11.3 What belongs in tools, tests or agent guidance

This is a migration decision table, not a new set of mandatory validators. Select meaningful constraints first. Each
mechanical rule has one configuration/runner owner, and its diagnostic explains remediation. Do not retain duplicated
prose to recite how a formatter or compiler works.

| Concern | Owning mechanism | What still requires judgment |
| --- | --- | --- |
| Whitespace, indentation, quotes, imports' formatting, final newlines | Shared Format entry point using the appropriate formatter per tracked text type, with intentional generated/vendor/evidence exclusions | Which files are authored, generated or historical captures; do not normalize raw evidence |
| Type errors, unused code, ordinary Go errors and React hook correctness | Compiler and selected Go/TypeScript/React lint rules in Lint | Whether ownership, lifecycle and error presentation are correct for the user |
| Unhandled promises and unsafe typed operations | Type-aware TypeScript lint with actual tsconfig ownership | Deadlines, acknowledgment, cancellation and safe fire-and-forget semantics |
| Adapter-only bridge imports and component dependency direction | One ESLint/import-boundary configuration; appropriate Go package/type checks | Where the real abstraction belongs and whether consumers use it |
| CSS syntax, token declarations/use, accidental color literals | Parsed CSS lint plus deliberate handling of inline TS/JS styles; central token generator/check where useful | Coherent surfaces, focus/keyboard behavior and theme invariance in the running UI |
| User-visible strings and accessibility syntax | Targeted static checks for supported source forms | Correct wording, roles, focus flow and usable native interactions |
| Generated bindings, dependency versions, file modes | Reproducible generation/build and pinned toolchain inputs | Compatibility and reasons for an upgrade; do not encode an incidental generator bug as permanent product policy |
| Allowed files/test ownership, excluded output and empty test collection | Runner/packaging configuration; fail when an expected test category collected nothing | Whether the tests exercise a meaningful outcome; never keep a DTO/paperwork test to satisfy a count |
| Safe writes, startup/retry/quit, offline bundled UI and real-backend parity | Behavioral unit/integration/E2E at the owning boundary, with build/runtime proof where needed | User-visible contract and failure/recovery decisions; no full native claim from a mock or an untagged build |
| Migrations and compatibility | Migration runner/integration checks; compare against the actual base revision where immutability is required | Safe data evolution and recovery; a clean `git diff HEAD` proves no branch history property |
| Test titles and comments | Optional precise lint for a forbidden identifier prefix if worthwhile; formatter for layout | A description explains behavior independently of specs; a comment explains a non-obvious contract, not history |
| KISS / DRY / SOLID and reusable UI/policy | Design review of existing owners, interfaces, consumers and behavior; no universal score | Which concerns change together, which differ, and whether an abstraction reduces real duplication |
| Agent scope, process and reporting | Short project-intent guidance plus actual shared commands | Proceed autonomously within authorization; surface real unresolved decisions and report measured results honestly |

Tool choices above were checked against current primary documentation, linked with their limits in
[agent-policy-tooling-sources.md](2026-09-07-project-health-evidence/agent-policy-tooling-sources.md). For example,
[typed linting](https://typescript-eslint.io/getting-started/typed-linting/) needs both type-aware rules and parser
project information; [TypeScript's resolution guidance](https://www.typescriptlang.org/tsconfig/moduleResolution)
distinguishes bundler and Node environments. These are configuration proposals, not claims that new rules were enabled.

#### 5.11.4 The agent guidance that should remain

A compact root policy should communicate the following, using current product and command pointers rather than
duplicated feature tables. This is proposed content, not a new live instruction file or a line quota.

- Build the authorized local Markdown editor capability. Preserve the agreed offline packaging, document durability,
  private local data, sanitization and accessible consistent interaction; current owner decisions resolve stale process.
- Inspect the existing behavior owner and its consumers before adding another implementation. Reuse common components,
  command policies, validation, persistence and lifecycle services; an intentional exception needs a concrete reason.
- Choose simple idiomatic code and cohesive responsibilities. Use composition and narrow replaceable interfaces where
  there is a real boundary. Keep one source of truth; avoid private copies and speculative frameworks.
- Perform routine reversible engineering work autonomously, planning in proportion to dependency and risk. Raise real
  product ambiguity or durable incompatible choices, and continue independent authorized work while they are resolved.
- Use the shared commands for mechanical feedback. Choose behavioral evidence appropriate to the changed outcome,
  including real native/backend execution when the claim depends on it. A successful mock, an unrun check or a
  completion marker cannot certify the shipped application.
- Preserve user work and describe the actual change, evidence and limits. A baseline distinguishes an existing failure
  from a regression; it does not turn an existing failure into acceptable behavior or excuse a newly introduced defect.

The why of backend ownership, safe close and common UI policy belongs here; exact import patterns, quote choices,
formatter flags and test-runner globs belong in their tools. Numeric **product** bounds and data-safety requirements
remain valid contracts. Removing arbitrary agent ceremony does not remove those product constraints.

#### 5.11.5 Migration without another competing policy layer

1. Update root guidance, the active constitution and project-owned templates together so the removed stops/quotas are
   not regenerated. Keep `CLAUDE.md` as a pointer plus necessary client mechanics; do not recreate redundant `rules/`
   trees just because older branches had them.
2. Retire unused legacy commands, pointer skills, symlinks and routing references as one change. For optional Spec Kit
   tools that remain, use supported project overrides/regeneration and verify their callable forms per client once;
   generated support files and manifests retain their actual owner. Remove unused hooks with their extensions.
3. Implement the useful mechanical checks in the existing five-script/six-stage design. Exercise real diagnostics,
   file coverage and exit codes; remove duplicate prose and ineffective substitutes after the check works. Do not
   introduce a new governance gate, restore deleted story validators, or switch stacks to solve configuration drift.
4. Validate one representative authorized change spanning common-component consumers: a fresh agent finds the current
   intent/owner, changes shared behavior once, chooses useful tests, runs the real commands and reports results without
   repeated routine approvals, copied specification clauses or unrequested automation.

## 6. Refactoring scope — input for the chosen planning workflow

This section supplies scope and acceptance for the refactoring, with any useful planning tool. It is not a task list
or a requirement to invoke Spec Kit. Requirement IDs `RF-…` are proposals for the new spec. Resolve §6.4 decisions
before their dependent implementation; they do not block independent authorized investigation or routine cleanup.
...
| RF-B13 | Decompose `App.tsx`, `DocumentTabs.tsx`, `ShellMenuRow.tsx` and `EditorChrome.tsx` around cohesive responsibilities and independent change: composition, command orchestration, drag, popup and shortcuts have identifiable owners. Small related callbacks may remain with their consumer; file-category or size quotas are not acceptance. The menubar is not rendered through the appearance controller. |
...
| RF-E7 | Lint includes the useful typed/import/CSS boundaries selected in §5.11; required suites reject accidental focus and empty collection. Source, tests, Node tools, generated files and reports have explicit compatible compiler/runner/exclusion ownership. |
| RF-E8 | CLI/library/toolchain versions and lockfile installation have one declared owner shared by local and CI. Pure-Go backend/SQLite checks are named and scoped accurately; the native Build stage validates the actual tagged Wails artifact and required host toolchain. |
...
| RF-F2 | Root agent guidance contains stable product intent, current authority/command pointers and the design judgment tools cannot supply. Remove duplicated mechanical rules, incident diaries, arbitrary line limits and required per-turn forms. Clarity is verified by a representative fresh-session task, not a new prose-counting/document-content gate. |
| RF-F3 | Retain only useful, supported optional commands and skills. One project-policy owner feeds the necessary client-specific wrappers; separately generated Codex/Claude invocation forms stay generator-owned. Retire unused legacy routes/extensions and all their hook references together. No compulsory Spec Kit or other ceremony for every authorized task. |
| RF-F4 | A concise current map identifies shared UI, command, lifecycle, persistence and verification owners. Record durable product/architecture decisions from §6.4; routine reversible implementation choices need no ADR or approval. |
| RF-F5 | Mechanical conventions and reliable static boundaries have one executable owner in Format/Lint/build validation; remove their duplicate instructions after the check works. Delete pointless paperwork constraints rather than moving them into a new linter. Keep product rationale and semantic ownership in guidance. |
| RF-F6 | Agents proceed through authorized diagnosis, implementation and relevant verification without task-path, rule-count, branch/commit or per-rule-test quotas. Changes remain reviewable and preserve user work. Genuine product ambiguity, destructive scope or durable incompatible tradeoffs are surfaced; unrelated missing paperwork does not block independent work. |
| RF-F7 | Reuse and current idioms are demonstrated by consumer adoption, explicit ownership and simpler supported code. Evaluate standard-library/framework improvements against the declared toolchain and real behavior; do not require speculative abstractions, indiscriminate upgrades or blanket memoization/interface patterns. |
| RF-F8 | If an autonomous runner is retained, it respects requested scope and host permissions, rejects failed child execution before accepting completion, preserves unrelated changes, and verifies actual outcomes. Disable unused auto-hooks/launchers instead of widening permissions or inventing a fallback workflow to keep them alive. |
...
1. Capture the baseline (this commit, gate results, defect list), record the current owner intent, and resolve D1–D12
   where they affect the next change. Record durable decisions; do not require an ADR or owner response for routine
   reversible details. Capture missing incident details in §5.10.6 without treating an unconfirmed cause as resolved.
...
6. Finish documentation and the architecture map when the working model is stable. Apply the Epic F autonomy,
   workflow-retirement and mechanical-rule ownership changes early enough that old instructions do not regenerate
   the rejected process during the refactor; keep command pointers synchronized as Epic E lands.
...
Revision-2.2 additions (8 September, same source commit):

| File | Content |
| --- | --- |
| `audit-revision-2.1-before-agent-review.md` | Exact audit before the agent/configuration extension, including the incident findings. |
| `agent-assets-inventory.txt`, `investigation-agent-assets.md` | Complete file/symlink inventory, hashes, generated ownership, command/skill dispositions and source-confirmed automation hazards. |
| `investigation-agent-governance.md` | Autonomy, copied-authority/test quotas, workflow/guard contradictions, useful remaining guidance and coordinated migration. |
| `investigation-mechanical-enforcement.md` | Detailed rule → current enforcer → gap → proposed owner matrix, actual CGO build-selection trace, config/version corrections and diagnostic limits. |
| `effective-eslint-agent-review.json` | Read-only resolved rule selection for normal frontend lint on App; not lint findings. |
| `agent-config-format-probe/` | Copied formatter config, unformatted/formatted inputs, raw diagnostic output and exact execution metadata; temporary module only. |
| `agent-policy-tooling-sources.md` | Current primary documentation supporting selective mechanical enforcement and modern idioms, with explicit limits. |

No live project instruction/config was changed. Earlier raw evidence, the owner's reviewed snapshots, the native
incident findings and Appendices A–D remain preserved. The evidence manifest includes these additions; it is not a
new project gate or specification authority.
