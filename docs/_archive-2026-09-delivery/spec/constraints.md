...
_Verified by:_ open the surface, switch theme three times and appearance twice, and look at it.
...
_Verified by:_ put the pointer down, tab through the surface, and use it.
...
| An open folder with nothing to show | `No Markdown files in this folder.` · _New file_ · _Open a different folder…_                                                               |
| A tree filter matching nothing      | `Nothing matches "<query>".` · _Clear filter_                                                                                               |
...
_Verified by:_ empty each list and read what it says.
...
_Verified by:_ search the changed files for quoted text that reaches the screen.
...
_Verified by:_ trigger each new failure path and read what appears.
...
_Verified by:_ make one thing fail repeatedly and count the toasts.
...
_Verified by:_ start each long operation, watch it, and cancel it.
...
Remote images and stylesheets referenced _inside a user's document_ are governed separately by the
...
_Verified by:_ a network monitor, for five minutes, with the app in use.
...
_Verified by:_ disconnect the network and use the app.
...
Examples: exactly 2.0 MB → live preview, because the check is _greater than_ 2 MB · 2.1 MB → paused ·
"bounded" with no number → not a specification.

_These are v1 numbers chosen to be safe, not tuned._ Raising one is a decision with evidence behind it.
Lowering one because something is slow is a defect being papered over.

_Verified by:_ cross each limit and read what happens.
...
_Verified by:_ look at the toolbar and the menus in a dark theme.
...
_Verified by:_ turn on reduced motion in the operating system and use the app.
...
_Verified by:_ type an out-of-range value and read the rejection.
...
_Verified by:_ add a command, then open the shortcuts dialog and the palette without touching either.
...
_Verified by:_ reading the live plan against what the phase added.
