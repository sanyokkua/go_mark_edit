...
7. **Prove the release candidate before shipping it.** Against a built artifact, not a dev server:
   ...
   bundled rather than fetched, startup within the budget in `../spec/constraints.md#every-limit-is-named`, and
   the Monaco bundle within its size bound. A candidate that misses one of these is not released.

## Where the details are

- Associations: `../spec/product/opening-files-from-the-desktop.md`
- Packaging and release: `../spec/product/opening-files-from-the-desktop.md`,
  `../architecture/stack.md`
- The budgets the candidate must meet: `../spec/constraints.md#every-limit-is-named`,
  `../spec/constraints.md#nothing-leaves-the-device`
- Text and languages: `../spec/product/language-and-text.md`
- Decisions: v1 ships unsigned and un-notarised; the git tag is the single source of the version,
  injected at link time into `internal/settings.AppVersion`; every platform icon derives from one
  source image; CI and development builds never touch production state
  (`../adr/0015-cicd-versioning-icon.md`)
- The icon source and script: `../../../build/icon/`
...
And the constraints every phase carries: confirm the About dialog shows the tag's version in the
released artifact and exactly `dev` in a local build; confirm the installed app works with the network
disconnected; watch the network for five minutes on the packaged build and confirm nothing is sent;
every new string goes through `t()`. All of it against the packaged artifact, not a development build.
