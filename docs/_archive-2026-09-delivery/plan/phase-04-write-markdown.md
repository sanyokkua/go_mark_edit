...
It was missing. `../spec/product/writing-in-the-editor.md:31` requires the formatting toolbar,
`../spec/product/keyboard-shortcuts.md` binds a shortcut to every one of them — and no phase implemented
...
- The action list: `../spec/product/writing-in-the-editor.md#editor-shows-source`
- The bindings: `../spec/product/keyboard-shortcuts.md` — the registry, and the format, file and view
...
- Editor settings: `../spec/product/settings.md#editor-group`
- What it looks like: `../spec/surface/mockup.html` → `editor-split` (toolbar), `context-menu`,
  `shortcuts`, `about`
- The seam to edit through: ADR-0017, and `logic/hooks/useDocumentCommands.ts`

## Questions to settle first

None blocking. `../spec/product/keyboard-shortcuts.md` already fixes every binding and scope.
...
it is what every other editor does — and write it into `../spec/product/writing-in-the-editor.md`.
...
And the constraints every phase carries: every toolbar control works in three themes across light and
dark and is reachable by keyboard alone with a visible focus ring; every label and tooltip goes through
`t()`; each new binding is registered once and appears in the menu, the tooltip and the shortcuts
dialog without any of them being edited; watch the network for five minutes and confirm nothing is
sent. All of it in a real build.
