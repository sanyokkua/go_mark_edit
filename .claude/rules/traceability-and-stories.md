...
The chain is: **spec clause -> story -> acceptance criterion -> test -> module**, recorded in the
...
- Front-matter (copy the schema in `02_STORY_FORMAT.md`) has, all required unless noted:
  ...
  resolving to a real heading), `modules` (>=1, each a path from `01_MODULE_INVENTORY.md`),
...
- Lifecycle: `draft -> ready -> in-progress -> done`; `done -> superseded`. `ready` requires front-matter
  to validate, every `depends_on` `done`, and every clause to resolve.
