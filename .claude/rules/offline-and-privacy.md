...
**Authority:** `specification/00_Foundation/04_DESIGN_DECISIONS.md` (DD-22, DD-32 *(revised for
Stage 3)*, DD-33, DD-34, DD-54), `00_Foundation/06_IMPLEMENTATION_STAGES.md` (F6 — offline invariant is
...
provider the user explicitly configured** (Stage 3, DD-32 revised / DD-38+), and **only on user action**
(invoking an action or sending a chat message). The **default provider is local** (e.g. Ollama / LM Studio),
so a default install stays fully on-device. **Stages 1–2 (Viewer, Editor) make zero network calls of any
...
- No **background/unsolicited** network call from the app: no update check, no analytics/telemetry ping, no
  ...
  user-configured provider** (Stage 3); it is never automatic, never background, and never to any other host.
  Stages 1–2 make no network call at all.
...
- [ ] No new *background/unsolicited* outbound network call; the only permitted outbound call is a
      user-invoked Stage-3 LLM inference to the configured provider; no runtime CDN reference.
